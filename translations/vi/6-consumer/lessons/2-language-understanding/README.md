...
Bạn có thể tạo các mô hình hiểu ngôn ngữ bằng cách sử dụng LUIS, một dịch vụ hiểu ngôn ngữ từ Microsoft thuộc Cognitive Services.
...
1. Đọc qua hướng dẫn xuất hiện sau khi bạn tạo ứng dụng để hiểu các bước bạn cần thực hiện để huấn luyện mô hình hiểu ngôn ngữ. Đóng hướng dẫn này khi bạn đã hoàn thành.

## Ý định và thực thể

Hiểu ngôn ngữ dựa trên *ý định* và *thực thể*. Ý định là mục đích của từ ngữ, ví dụ như phát nhạc, đặt hẹn giờ, hoặc gọi món ăn. Thực thể là những gì ý định đang đề cập đến, chẳng hạn như album, thời gian hẹn giờ, hoặc loại thức ăn. Mỗi câu mà mô hình diễn giải nên có ít nhất một ý định, và tùy chọn một hoặc nhiều thực thể.
...
Khi các thực thể được xác định, bạn tạo các ý định. Những ý định này được mô hình học dựa trên các câu ví dụ mà bạn cung cấp (được gọi là các câu nói). Ví dụ, đối với ý định *đặt hẹn giờ*, bạn có thể cung cấp các câu sau:
...
> 💁 Như với bất kỳ mô hình AI nào, dữ liệu càng nhiều và càng chính xác mà bạn sử dụng để huấn luyện, mô hình càng tốt.
...
Bạn có thể tìm thấy hướng dẫn sử dụng cổng LUIS trong tài liệu [Quickstart: Build your app in LUIS portal trên Microsoft docs](https://docs.microsoft.com/azure/cognitive-services/luis/luis-get-started-create-app?WT.mc_id=academic-17441-jabenn).
...
1. Khi các thực thể và ý định được cấu hình, bạn có thể huấn luyện mô hình bằng cách sử dụng nút **Huấn luyện** trên menu trên cùng. Chọn nút này, và mô hình sẽ được huấn luyện trong vài giây. Nút sẽ bị vô hiệu hóa trong khi huấn luyện, và được kích hoạt lại khi hoàn thành.
...
1. Đóng bảng *Kiểm tra* khi bạn đã hoàn thành kiểm tra.

### Nhiệm vụ - xuất bản mô hình

Để sử dụng mô hình này từ mã, bạn cần xuất bản nó. Khi xuất bản từ LUIS, bạn có thể xuất bản đến môi trường staging để kiểm tra, hoặc môi trường sản phẩm để phát hành đầy đủ. Trong bài học này, môi trường staging là đủ.

1. Từ cổng LUIS, chọn nút **Xuất bản** từ menu trên cùng.

1. Đảm bảo *Staging slot* được chọn, sau đó chọn **Hoàn tất**. Bạn sẽ thấy thông báo khi ứng dụng được xuất bản.

1. Bạn có thể kiểm tra điều này bằng cách sử dụng curl. Để xây dựng lệnh curl, bạn cần ba giá trị - endpoint, ID ứng dụng (App ID) và một API key. Những giá trị này có thể được truy cập từ tab **QUẢN LÝ** có thể được chọn từ menu trên cùng.
...
1. Kết quả của lệnh này sẽ là một tài liệu JSON chi tiết truy vấn, intent chính, và danh sách các entity được phân loại theo loại.
    ...
    * `set timer` là intent chính với xác suất 97%.
    * Hai entity *number* được phát hiện: `45` và `12`.
    * Hai entity *time-unit* được phát hiện: `minute` và `second`.

## Sử dụng mô hình hiểu ngôn ngữ

Khi đã được xuất bản, mô hình LUIS có thể được gọi từ mã. Trong các bài học trước, bạn đã sử dụng IoT Hub để xử lý giao tiếp với các dịch vụ đám mây, gửi dữ liệu telemetry và lắng nghe các lệnh. Điều này rất không đồng bộ - khi dữ liệu telemetry được gửi đi, mã của bạn không chờ phản hồi, và nếu dịch vụ đám mây bị gián đoạn, bạn sẽ không biết.

Đối với một bộ hẹn giờ thông minh, chúng ta muốn có phản hồi ngay lập tức để thông báo cho người dùng rằng bộ hẹn giờ đã được đặt, hoặc cảnh báo rằng dịch vụ đám mây không khả dụng. Để làm điều này, thiết bị IoT của chúng ta sẽ gọi trực tiếp một endpoint web, thay vì dựa vào IoT Hub.

Thay vì gọi LUIS từ thiết bị IoT, bạn có thể sử dụng mã serverless với một loại trigger khác - HTTP trigger. Điều này cho phép ứng dụng function của bạn lắng nghe các yêu cầu REST và phản hồi chúng. Chức năng này sẽ là một REST endpoint mà thiết bị của bạn có thể gọi.

> 💁 Mặc dù bạn có thể gọi LUIS trực tiếp từ thiết bị IoT của mình, nhưng tốt hơn là sử dụng một thứ như mã serverless. Bằng cách này, khi bạn muốn thay đổi ứng dụng LUIS mà bạn gọi, ví dụ khi bạn huấn luyện một mô hình tốt hơn hoặc huấn luyện một mô hình bằng ngôn ngữ khác, bạn chỉ cần cập nhật mã đám mây của mình, thay vì triển khai lại mã cho hàng nghìn hoặc hàng triệu thiết bị IoT.
...
1. Thêm một HTTP trigger vào ứng dụng này có tên `speech-trigger` bằng cách sử dụng lệnh sau trong terminal của VS Code:
...
1. Kiểm tra HTTP trigger bằng cách chạy ứng dụng functions. Khi chạy, bạn sẽ thấy endpoint được liệt kê trong output:
...
1. SDK cho LUIS có sẵn thông qua một gói Pip. Thêm dòng sau vào tệp `requirements.txt` để thêm phụ thuộc vào gói này:
...
1. Phản hồi dự đoán chứa intent chính - intent có điểm dự đoán cao nhất, cùng với các entity. Nếu intent chính là `set timer`, thì các entity có thể được đọc để lấy thời gian cần thiết cho bộ hẹn giờ:
    ...
    Các entity `number` sẽ là một mảng các số. Ví dụ, nếu bạn nói *"Set a four minute 17 second timer."*, thì mảng `number` sẽ chứa 2 số nguyên - 4 và 17.

    Các entity `time unit` sẽ là một mảng các mảng chuỗi, với mỗi đơn vị thời gian là một mảng chuỗi bên trong mảng. Ví dụ, nếu bạn nói *"Set a four minute 17 second timer."*, thì mảng `time unit` sẽ chứa 2 mảng với mỗi mảng có một giá trị - `['minute']` và `['second']`.

    Phiên bản JSON của các entity này cho *"Set a four minute 17 second timer."* là:
    ...
    Đoạn mã này cũng định nghĩa một biến đếm cho tổng thời gian của bộ hẹn giờ tính bằng giây. Biến này sẽ được điền giá trị từ các entity.

1. Các entity không được liên kết, nhưng chúng ta có thể đưa ra một số giả định về chúng. Chúng sẽ theo thứ tự được nói, vì vậy vị trí trong mảng có thể được sử dụng để xác định số nào khớp với đơn vị thời gian nào. Ví dụ:
    ...
    * *"Set a 2 minute and 30 second timer"* - điều này sẽ có hai số, `2` và `30`, và hai đơn vị thời gian, `minute` và `second`, vì vậy số đầu tiên sẽ dành cho đơn vị thời gian đầu tiên (2 phút), và số thứ hai cho đơn vị thời gian thứ hai (30 giây).
    ...
    Đoạn mã sau lấy số lượng mục trong các entity số, và sử dụng số này để trích xuất mục đầu tiên từ mỗi mảng, sau đó là mục thứ hai, và cứ tiếp tục như vậy. Thêm đoạn mã này vào bên trong khối `if`.
    ...
    Đối với *"Set a four minute 17 second timer."*, đoạn mã này sẽ lặp lại hai lần, cho các giá trị sau:
...
1. Bên ngoài vòng lặp qua các entity, ghi log tổng thời gian cho bộ hẹn giờ:
...
1. Cuối cùng, bên ngoài khối `if`, xử lý trường hợp intent không được nhận diện bằng cách trả về mã lỗi:

    ```python
    return func.HttpResponse(status_code=404)
    ```

    404 là mã trạng thái cho *not found*.
...
1. Chạy ứng dụng function và kiểm tra nó bằng cách sử dụng curl.
    ...
    Bạn sẽ thấy output sau từ ứng dụng function:
    ...
    Lệnh curl sẽ trả về kết quả sau:
...
1. Để thiết bị IoT của bạn gọi REST endpoint, nó sẽ cần biết URL. Khi bạn truy cập nó trước đó, bạn đã sử dụng `localhost`, đây là một cách tắt để truy cập REST endpoint trên máy cục bộ của bạn. Để thiết bị IoT của bạn có thể truy cập, bạn cần hoặc xuất bản lên đám mây, hoặc lấy địa chỉ IP của bạn để truy cập cục bộ.

    > ⚠️ Nếu bạn đang sử dụng Wio Terminal, dễ dàng hơn để chạy ứng dụng function cục bộ, vì sẽ có phụ thuộc vào các thư viện khiến bạn không thể triển khai ứng dụng function theo cách bạn đã làm trước đó. Chạy ứng dụng function cục bộ và truy cập nó qua địa chỉ IP của máy tính của bạn. Nếu bạn muốn triển khai lên đám mây, thông tin sẽ được cung cấp trong bài học sau về cách thực hiện điều này.
    ...
    * Xuất bản ứng dụng Functions - làm theo hướng dẫn trong các bài học trước để xuất bản ứng dụng functions của bạn lên đám mây. Khi đã xuất bản, URL sẽ là `https://<APP_NAME>.azurewebsites.net/api/text-to-timer`, trong đó `<APP_NAME>` sẽ là tên của ứng dụng functions của bạn. Đảm bảo cũng xuất bản các cài đặt cục bộ của bạn.
      ...
      Khi làm việc với HTTP trigger, chúng được bảo mật mặc định với một function app key. Để lấy key này, chạy lệnh sau:
      ...
      > 💁 Bạn có thể thay đổi loại ủy quyền của HTTP trigger bằng cách sử dụng cài đặt `authlevel` trong tệp `function.json`. Bạn có thể đọc thêm về điều này trong [phần cấu hình của tài liệu HTTP trigger Azure Functions trên Microsoft docs](https://docs.microsoft.com/azure/azure-functions/functions-bindings-http-webhook-trigger?WT.mc_id=academic-17441-jabenn&tabs=python#configuration).

    * Chạy ứng dụng functions cục bộ, và truy cập bằng địa chỉ IP - bạn có thể lấy địa chỉ IP của máy tính trên mạng cục bộ, và sử dụng nó để xây dựng URL.
...
:7071/api/text-to-timer`, trong đó `<IP_ADDRESS>` sẽ là địa chỉ IP của bạn, ví dụ `http://192.168.1.10:7071/api/text-to-timer`.
...
Có nhiều cách để yêu cầu cùng một việc, chẳng hạn như đặt hẹn giờ. Hãy nghĩ ra các cách khác nhau để làm điều này và sử dụng chúng làm ví dụ trong ứng dụng LUIS của bạn. Thử nghiệm những cách này để xem mô hình của bạn có thể xử lý tốt như thế nào với nhiều cách yêu cầu hẹn giờ khác nhau.
...
* Đọc thêm về hiểu ngôn ngữ tự nhiên trên [trang hiểu ngôn ngữ tự nhiên trên Wikipedia](https://wikipedia.org/wiki/Natural-language_understanding)
* Đọc thêm về HTTP triggers trong [tài liệu HTTP trigger của Azure Functions trên Microsoft docs](https://docs.microsoft.com/azure/azure-functions/functions-bindings-http-webhook-trigger?WT.mc_id=academic-17441-jabenn&tabs=python)
