...
Trong phần này của bài học, bạn sẽ chuyển đổi văn bản thành giọng nói để cung cấp phản hồi bằng âm thanh.
...
1. Thêm một HTTP trigger mới vào ứng dụng này có tên `get-voices` bằng cách sử dụng lệnh sau từ bên trong terminal của VS Code trong thư mục gốc của dự án ứng dụng functions:
...
1. Chạy ứng dụng function của bạn cục bộ. Sau đó, bạn có thể gọi điều này bằng một công cụ như curl theo cách tương tự như bạn đã thử nghiệm HTTP trigger `text-to-timer`. Đảm bảo truyền ngôn ngữ của bạn dưới dạng nội dung JSON:
...
1. Mở tệp tiêu đề `config.h` và thêm URL cho ứng dụng function của bạn:

    ```cpp
    const char *GET_VOICES_FUNCTION_URL = "<URL>";
    ```

    Thay thế `<URL>` bằng URL cho HTTP trigger `get-voices` trên ứng dụng function của bạn. Điều này sẽ giống với giá trị của `TEXT_TO_TIMER_FUNCTION_URL`, ngoại trừ tên hàm là `get-voices` thay vì `text-to-timer`.
...
1. Để gọi ứng dụng function của bạn, bạn cần khai báo một WiFi client. Thêm đoạn mã sau vào phần `private` của lớp:
...
1. Để lấy các giọng nói, một tài liệu JSON cần được gửi đến ứng dụng function với ngôn ngữ. Thêm đoạn mã sau vào hàm `init` để tạo tài liệu JSON này:
...
1. Tiếp theo, tạo một `HTTPClient`, sau đó sử dụng nó để gọi ứng dụng function để lấy các giọng nói, gửi tài liệu JSON:
...
1. Biên dịch mã này, tải nó lên Wio Terminal của bạn và thử nghiệm thông qua serial monitor. Đảm bảo ứng dụng function của bạn đang chạy.

    Bạn sẽ thấy danh sách các giọng nói có sẵn được trả về từ ứng dụng function, cùng với giọng nói đã chọn.
...
Khi bạn đã có một giọng nói để sử dụng, nó có thể được sử dụng để chuyển đổi văn bản thành giọng nói. Các hạn chế về bộ nhớ với các giọng nói cũng áp dụng khi chuyển đổi giọng nói thành văn bản, vì vậy bạn sẽ cần ghi giọng nói vào thẻ SD để phát qua ReSpeaker.

> 💁 Trong các bài học trước của dự án này, bạn đã sử dụng bộ nhớ flash để lưu trữ giọng nói được thu từ micro. Bài học này sử dụng thẻ SD vì nó dễ dàng hơn để phát âm thanh từ đó bằng cách sử dụng thư viện âm thanh của Seeed.

Ngoài ra còn có một hạn chế khác cần xem xét, dữ liệu âm thanh có sẵn từ dịch vụ giọng nói và các định dạng mà Wio Terminal hỗ trợ. Không giống như máy tính đầy đủ, các thư viện âm thanh cho vi điều khiển có thể rất hạn chế về các định dạng âm thanh mà chúng hỗ trợ. Ví dụ, thư viện Seeed Arduino Audio có thể phát âm thanh qua ReSpeaker chỉ hỗ trợ âm thanh ở tần số mẫu 44.1KHz. Dịch vụ giọng nói Azure có thể cung cấp âm thanh ở một số định dạng, nhưng không định dạng nào sử dụng tần số mẫu này, chúng chỉ cung cấp 8KHz, 16KHz, 24KHz và 48KHz. Điều này có nghĩa là âm thanh cần được chuyển đổi lại thành 44.1KHz, điều mà sẽ cần nhiều tài nguyên hơn Wio Terminal có, đặc biệt là bộ nhớ.

Khi cần thao tác dữ liệu như thế này, thường tốt hơn là sử dụng mã serverless, đặc biệt nếu dữ liệu được lấy thông qua một cuộc gọi web. Wio Terminal có thể gọi một hàm serverless, truyền văn bản để chuyển đổi, và hàm serverless có thể vừa gọi dịch vụ giọng nói để chuyển đổi văn bản thành giọng nói, vừa chuyển đổi lại âm thanh thành tần số mẫu cần thiết. Sau đó, nó có thể trả về âm thanh dưới dạng mà Wio Terminal cần để lưu trữ trên thẻ SD và phát qua ReSpeaker.

### Nhiệm vụ - tạo một hàm serverless để chuyển đổi văn bản thành giọng nói

1. Mở dự án `smart-timer-trigger` của bạn trong VS Code, và mở terminal đảm bảo môi trường ảo đã được kích hoạt. Nếu không, hãy tắt và tạo lại terminal.

1. Thêm một HTTP trigger mới vào ứng dụng này có tên `text-to-speech` bằng cách sử dụng lệnh sau từ bên trong terminal của VS Code trong thư mục gốc của dự án ứng dụng functions:
...
1. Gói Pip [librosa](https://librosa.org) có các hàm để chuyển đổi lại âm thanh, vì vậy hãy thêm gói này vào tệp `requirements.txt`:
...
1. Chạy ứng dụng function của bạn cục bộ, hoặc triển khai nó lên cloud. Sau đó, bạn có thể gọi điều này bằng một công cụ như curl theo cách tương tự như bạn đã thử nghiệm HTTP trigger `text-to-timer`. Đảm bảo truyền ngôn ngữ, giọng nói và văn bản dưới dạng nội dung JSON:
    ...
    Ví dụ, để chuyển đổi "Hello" thành giọng nói sử dụng tiếng Anh Mỹ với giọng Jenny Neural, với ứng dụng function chạy cục bộ, bạn có thể sử dụng lệnh curl sau:
...
1. Mở tệp tiêu đề `config.h` và thêm URL cho ứng dụng function của bạn:
    ...
    Thay thế `<URL>` bằng URL cho HTTP trigger `text-to-speech` trên ứng dụng function của bạn. Điều này sẽ giống với giá trị của `TEXT_TO_TIMER_FUNCTION_URL`, ngoại trừ tên hàm là `text-to-speech` thay vì `text-to-timer`.
...
1. Trong phương thức `convertTextToSpeech`, thêm đoạn mã sau để tạo JSON gửi đến ứng dụng function:
...
1. Bên dưới đoạn mã này, thêm đoạn mã sau để gọi ứng dụng function:
    ...
    Điều này tạo một HTTPClient, sau đó thực hiện một yêu cầu POST sử dụng tài liệu JSON đến HTTP trigger chuyển đổi văn bản thành giọng nói.

1. Nếu cuộc gọi thành công, dữ liệu nhị phân thô được trả về từ cuộc gọi ứng dụng function có thể được truyền trực tiếp vào một tệp trên thẻ SD. Thêm đoạn mã sau để thực hiện điều này:
...
## Triển khai ứng dụng functions của bạn lên đám mây

Lý do để chạy ứng dụng functions cục bộ là vì gói Pip `librosa` trên Linux có một phụ thuộc vào một thư viện không được cài đặt mặc định, và thư viện này cần được cài đặt trước khi ứng dụng function có thể chạy. Các ứng dụng function là serverless - không có máy chủ nào mà bạn có thể tự quản lý, vì vậy không có cách nào để cài đặt thư viện này trước.

Cách để làm điều này là triển khai ứng dụng functions của bạn bằng cách sử dụng một container Docker. Container này sẽ được triển khai bởi đám mây bất cứ khi nào cần khởi động một phiên bản mới của ứng dụng function của bạn (chẳng hạn khi nhu cầu vượt quá tài nguyên sẵn có, hoặc nếu ứng dụng function không được sử dụng trong một thời gian và bị đóng lại).

Bạn có thể tìm thấy hướng dẫn để thiết lập một ứng dụng function và triển khai qua Docker trong [tài liệu tạo function trên Linux bằng container tùy chỉnh trên Microsoft Docs](https://docs.microsoft.com/azure/azure-functions/functions-create-function-linux-custom-image?WT.mc_id=academic-17441-jabenn&tabs=bash%2Cazurecli&pivots=programming-language-python).

Sau khi triển khai xong, bạn có thể chuyển mã Wio Terminal của mình để truy cập function này:
...
1. Thay đổi tất cả các include từ `<WiFiClient.h>` thành `<WiFiClientSecure.h>`.

1. Thay đổi tất cả các trường `WiFiClient` thành `WiFiClientSecure`.

1. Trong mỗi lớp có trường `WiFiClientSecure`, thêm một constructor và thiết lập chứng chỉ trong constructor đó:
