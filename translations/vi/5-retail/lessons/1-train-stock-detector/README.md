...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:55:02+00:00",
...
![Tóm tắt bài học bằng hình vẽ](../../../../../translated_images/lesson-19.cf6973cecadf080c4b526310620dc4d6f5994c80fb0139c6f378cc9ca2d435cd.vi.jpg)

> Hình vẽ minh họa bởi [Nitya Narasimhan](https://github.com/nitya). Nhấp vào hình để xem phiên bản lớn hơn.

Video này cung cấp cái nhìn tổng quan về Phát hiện Đối tượng bằng dịch vụ Azure Custom Vision, một dịch vụ sẽ được đề cập trong bài học này.
...
Trong dự án trước, bạn đã sử dụng AI để huấn luyện một bộ phân loại hình ảnh - một mô hình có thể xác định xem một hình ảnh có chứa thứ gì đó, chẳng hạn như trái cây chín hoặc chưa chín. Một loại mô hình AI khác có thể được sử dụng với hình ảnh là phát hiện đối tượng. Các mô hình này không phân loại hình ảnh bằng thẻ, thay vào đó chúng được huấn luyện để nhận diện các đối tượng và có thể tìm thấy chúng trong hình ảnh, không chỉ phát hiện rằng đối tượng có mặt mà còn phát hiện vị trí của nó trong hình ảnh. Điều này cho phép bạn đếm các đối tượng trong hình ảnh.
...
Phân loại hình ảnh là phân loại toàn bộ hình ảnh - xác suất rằng toàn bộ hình ảnh khớp với từng thẻ là bao nhiêu. Bạn sẽ nhận được các xác suất cho mỗi thẻ được sử dụng để huấn luyện mô hình.
...
Bạn có thể sử dụng các giá trị này với một ngưỡng phần trăm để dự đoán nội dung trong hình ảnh. Nhưng nếu một hình ảnh chứa nhiều lon sốt cà chua, hoặc cả hạt điều và sốt cà chua? Kết quả có thể sẽ không mang lại điều bạn mong muốn. Đây là lúc phát hiện đối tượng trở nên hữu ích.

Phát hiện đối tượng liên quan đến việc huấn luyện một mô hình để nhận diện các đối tượng. Thay vì cung cấp cho nó các hình ảnh chứa đối tượng và nói rằng mỗi hình ảnh là một thẻ này hay thẻ khác, bạn đánh dấu phần của hình ảnh chứa đối tượng cụ thể và gắn thẻ đó. Bạn có thể gắn thẻ một đối tượng duy nhất trong một hình ảnh hoặc nhiều đối tượng. Bằng cách này, mô hình học được hình dạng của chính đối tượng, không chỉ là hình ảnh chứa đối tượng trông như thế nào.

Khi bạn sử dụng nó để dự đoán hình ảnh, thay vì nhận được danh sách các thẻ và phần trăm, bạn sẽ nhận được danh sách các đối tượng được phát hiện, với hộp giới hạn và xác suất rằng đối tượng khớp với thẻ được gắn.

> 🎓 *Hộp giới hạn* là các hộp xung quanh một đối tượng.
...
Hình ảnh trên chứa cả một hộp hạt điều và ba lon sốt cà chua. Bộ phát hiện đối tượng đã phát hiện hạt điều, trả về hộp giới hạn chứa hạt điều với xác suất rằng hộp giới hạn chứa đối tượng, trong trường hợp này là 97.6%. Bộ phát hiện đối tượng cũng đã phát hiện ba lon sốt cà chua và cung cấp ba hộp giới hạn riêng biệt, một cho mỗi lon được phát hiện, và mỗi hộp có một xác suất rằng hộp giới hạn chứa một lon sốt cà chua.

✅ Hãy nghĩ về một số tình huống khác nhau mà bạn có thể muốn sử dụng các mô hình AI dựa trên hình ảnh. Tình huống nào cần phân loại, và tình huống nào cần phát hiện đối tượng?

### Cách hoạt động của phát hiện đối tượng

Phát hiện đối tượng sử dụng các mô hình học máy phức tạp. Các mô hình này hoạt động bằng cách chia hình ảnh thành nhiều ô, sau đó kiểm tra xem trung tâm của hộp giới hạn có phải là trung tâm của một hình ảnh khớp với một trong các hình ảnh được sử dụng để huấn luyện mô hình hay không. Bạn có thể nghĩ điều này giống như chạy một bộ phân loại hình ảnh trên các phần khác nhau của hình ảnh để tìm kiếm sự khớp.

> 💁 Đây là một sự đơn giản hóa rất lớn. Có nhiều kỹ thuật khác nhau để phát hiện đối tượng, và bạn có thể đọc thêm về chúng trên [trang Phát hiện đối tượng trên Wikipedia](https://wikipedia.org/wiki/Object_detection).
...
* **Kiểm tra và đếm hàng hóa** - nhận diện khi hàng hóa trên kệ bị thiếu. Nếu hàng hóa quá ít, thông báo có thể được gửi đến nhân viên hoặc robot để bổ sung hàng.
* **Phát hiện khẩu trang** - trong các cửa hàng có chính sách đeo khẩu trang trong các sự kiện y tế công cộng, phát hiện đối tượng có thể nhận diện người có đeo khẩu trang và người không đeo.
...
* **Phát hiện nguy hiểm** - nhận diện các vật dụng bị vỡ trên sàn hoặc chất lỏng bị đổ, thông báo cho đội vệ sinh.

✅ Nghiên cứu thêm: Một số ứng dụng khác của phát hiện đối tượng trong bán lẻ là gì?
...
1. Làm theo [phần Tạo một dự án mới trong hướng dẫn nhanh Xây dựng bộ phát hiện đối tượng trên tài liệu Microsoft](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/get-started-build-detector?WT.mc_id=academic-17441-jabenn#create-a-new-project) để tạo một dự án Custom Vision mới. Giao diện người dùng có thể thay đổi và tài liệu này luôn là tài liệu tham khảo cập nhật nhất.
    ...
    Khi bạn tạo dự án của mình, hãy đảm bảo sử dụng tài nguyên `stock-detector-training` bạn đã tạo trước đó. Sử dụng loại dự án *Phát hiện Đối tượng* và miền *Sản phẩm trên Kệ*.
...
1. Thu thập hình ảnh chứa đối tượng cần phát hiện. Bạn sẽ cần ít nhất 15 hình ảnh chứa mỗi đối tượng cần phát hiện từ nhiều góc độ khác nhau và trong các điều kiện ánh sáng khác nhau, nhưng càng nhiều càng tốt. Bộ phát hiện đối tượng này sử dụng miền *Sản phẩm trên kệ*, vì vậy hãy cố gắng sắp xếp các đối tượng như thể chúng đang trên kệ cửa hàng. Bạn cũng sẽ cần một vài hình ảnh để kiểm tra mô hình. Nếu bạn đang phát hiện nhiều hơn một đối tượng, bạn sẽ muốn một số hình ảnh kiểm tra chứa tất cả các đối tượng.

    > 💁 Hình ảnh với nhiều đối tượng khác nhau được tính vào tối thiểu 15 hình ảnh cho tất cả các đối tượng trong hình ảnh.

    Hình ảnh của bạn nên là png hoặc jpeg, nhỏ hơn 6MB. Nếu bạn tạo chúng bằng iPhone chẳng hạn, chúng có thể là hình ảnh HEIC độ phân giải cao, vì vậy sẽ cần được chuyển đổi và có thể thu nhỏ. Càng nhiều hình ảnh càng tốt, và bạn nên có số lượng tương tự của trái cây chín và chưa chín.

    Mô hình được thiết kế cho các sản phẩm trên kệ, vì vậy hãy cố gắng chụp ảnh các đối tượng trên kệ.
...
1. Làm theo [phần Tải lên và gắn thẻ hình ảnh trong hướng dẫn nhanh Xây dựng bộ phát hiện đối tượng trên tài liệu Microsoft](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/get-started-build-detector?WT.mc_id=academic-17441-jabenn#upload-and-tag-images) để tải lên hình ảnh huấn luyện của bạn. Tạo các thẻ phù hợp tùy thuộc vào loại đối tượng bạn muốn phát hiện.
    ...
    Khi bạn vẽ các hộp giới hạn cho các đối tượng, hãy giữ chúng thật chặt xung quanh đối tượng. Có thể mất một thời gian để đánh dấu tất cả các hình ảnh, nhưng công cụ sẽ phát hiện những gì nó nghĩ là các hộp giới hạn, giúp nhanh hơn.

    ![Gắn thẻ sốt cà chua](../../../../../translated_images/object-detector-tag-tomato-paste.f47c362fb0f0eb582f3bc68cf3855fb43a805106395358d41896a269c210b7b4.vi.png)

    > 💁 Nếu bạn có hơn 15 hình ảnh cho mỗi đối tượng, bạn có thể huấn luyện sau 15 hình ảnh sau đó sử dụng tính năng **Thẻ gợi ý**. Điều này sẽ sử dụng mô hình đã huấn luyện để phát hiện các đối tượng trong hình ảnh chưa được gắn thẻ. Bạn có thể xác nhận các đối tượng được phát hiện hoặc từ chối và vẽ lại các hộp giới hạn. Điều này có thể tiết kiệm rất nhiều thời gian.
...
1. Làm theo [phần Huấn luyện bộ phát hiện trong hướng dẫn nhanh Xây dựng bộ phát hiện đối tượng trên tài liệu Microsoft](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/get-started-build-detector?WT.mc_id=academic-17441-jabenn#train-the-detector) để huấn luyện bộ phát hiện đối tượng trên các hình ảnh đã gắn thẻ của bạn.
    ...
    Bạn sẽ được cung cấp một lựa chọn loại huấn luyện. Chọn **Huấn luyện nhanh**.

Bộ phát hiện đối tượng sau đó sẽ được huấn luyện. Quá trình huấn luyện sẽ mất vài phút để hoàn thành.
...
1. Sử dụng nút **Kiểm tra nhanh** để tải lên các hình ảnh kiểm tra và xác minh các đối tượng được phát hiện. Sử dụng các hình ảnh kiểm tra bạn đã tạo trước đó, không phải bất kỳ hình ảnh nào bạn đã sử dụng để huấn luyện.
...
Khi bạn kiểm tra bộ phát hiện đối tượng của mình, nó có thể không mang lại kết quả như mong đợi, giống như với các bộ phân loại hình ảnh trong dự án trước. Bạn có thể cải thiện bộ phát hiện đối tượng của mình bằng cách huấn luyện lại nó với các hình ảnh mà nó nhận diện sai.

Mỗi khi bạn thực hiện một dự đoán bằng tùy chọn kiểm tra nhanh, hình ảnh và kết quả sẽ được lưu trữ. Bạn có thể sử dụng các hình ảnh này để huấn luyện lại mô hình của mình.
...
Điều gì sẽ xảy ra nếu bạn sử dụng bộ phát hiện đối tượng với các mặt hàng trông giống nhau, chẳng hạn như các lon sốt cà chua và cà chua cắt nhỏ cùng thương hiệu?

Nếu bạn có bất kỳ mặt hàng nào trông giống nhau, hãy thử nghiệm bằng cách thêm hình ảnh của chúng vào bộ phát hiện đối tượng của bạn.
...
* Khi bạn huấn luyện bộ phát hiện đối tượng, bạn sẽ thấy các giá trị như *Precision*, *Recall*, và *mAP* đánh giá mô hình đã được tạo. Tìm hiểu thêm về các giá trị này bằng cách đọc [phần Đánh giá bộ phát hiện trong hướng dẫn nhanh Xây dựng bộ phát hiện đối tượng trên tài liệu Microsoft](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/get-started-build-detector?WT.mc_id=academic-17441-jabenn#evaluate-the-detector)
