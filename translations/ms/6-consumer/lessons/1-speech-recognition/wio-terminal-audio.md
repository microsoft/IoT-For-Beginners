...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:33:49+00:00",
  "source_file": "6-consumer/lessons/1-speech-recognition/wio-terminal-audio.md",
  "language_code": "ms"
}
-->
# Tangkap Audio - Wio Terminal

Dalam bahagian pelajaran ini, anda akan menulis kod untuk menangkap audio pada Wio Terminal anda. Tangkap audio akan dikawal oleh salah satu butang di bahagian atas Wio Terminal.

## Programkan peranti untuk menangkap audio

Anda boleh menangkap audio dari mikrofon menggunakan kod C++. Wio Terminal hanya mempunyai 192KB RAM, tidak cukup untuk menangkap lebih daripada beberapa saat audio. Ia juga mempunyai 4MB memori flash, jadi ini boleh digunakan sebagai alternatif, dengan menyimpan audio yang ditangkap ke memori flash.
...
![Audio dari mikrofon pergi ke ADC kemudian ke DMAC. DMAC menulis ke satu buffer. Apabila buffer ini penuh, ia diproses dan DMAC menulis ke buffer kedua](../../../../../translated_images/dmac-adc-buffers.4509aee49145c90bc2e1be472b8ed2ddfcb2b6a81ad3e559114aca55f5fff759.ms.png)

DMAC boleh menangkap audio dari ADC pada selang masa tetap, seperti 16,000 kali sesaat untuk audio 16KHz. Ia boleh menulis data yang ditangkap ini ke buffer memori yang telah diperuntukkan, dan apabila buffer ini penuh, ia tersedia untuk kod anda untuk diproses. Menggunakan memori ini boleh melambatkan penangkapan audio, tetapi anda boleh menyediakan beberapa buffer. DMAC menulis ke buffer 1, kemudian apabila ia penuh, memberitahu kod anda untuk memproses buffer 1, sementara DMAC menulis ke buffer 2. Apabila buffer 2 penuh, ia memberitahu kod anda, dan kembali menulis ke buffer 1. Dengan cara ini, selagi anda memproses setiap buffer dalam masa yang lebih singkat daripada masa yang diperlukan untuk mengisi satu buffer, anda tidak akan kehilangan sebarang data.

Apabila setiap buffer telah ditangkap, ia boleh ditulis ke memori flash. Memori flash perlu ditulis menggunakan alamat yang ditentukan, menentukan di mana untuk menulis dan berapa besar untuk ditulis, serupa dengan mengemas kini array byte dalam memori. Memori flash mempunyai granulariti, yang bermaksud operasi penghapusan dan penulisan bergantung bukan sahaja pada saiz tetap, tetapi juga penjajaran kepada saiz tersebut. Contohnya, jika granulariti adalah 4096 byte dan anda meminta penghapusan pada alamat 4200, ia boleh menghapus semua data dari alamat 4096 hingga 8192. Ini bermaksud apabila anda menulis data audio ke memori flash, ia perlu dalam bahagian yang bersaiz betul.

### Tugas - konfigurasikan memori flash
...
1. Dalam bahagian `private`, tambahkan kod berikut:
    ...
    Kod ini mendefinisikan beberapa medan untuk buffer yang digunakan untuk menyimpan data sebelum menulisnya ke memori flash. Terdapat array byte, `_sfudBuffer`, untuk menulis data, dan apabila buffer ini penuh, data ditulis ke memori flash. Medan `_sfudBufferPos` menyimpan lokasi semasa untuk menulis dalam buffer ini, dan `_sfudBufferWritePos` menyimpan lokasi dalam memori flash untuk menulis. `_flash` adalah penunjuk kepada memori flash untuk ditulis - beberapa mikrokontroler mempunyai beberapa cip memori flash.
...
1. Tambahkan kaedah berikut ke bahagian `public` untuk menginisialisasi kelas ini:
    ...
    Kod ini mengkonfigurasi memori flash pada Wio Terminal untuk ditulis, dan menyediakan buffer berdasarkan saiz granulariti memori flash. Ini berada dalam kaedah `init`, bukan konstruktor kerana ini perlu dipanggil selepas memori flash disediakan dalam fungsi `setup`.
...
1. Tambahkan kod berikut ke bahagian `public`:
    ...
    Kod ini mendefinisikan kaedah untuk menulis byte ke sistem penyimpanan flash. Ia berfungsi dengan menulis ke buffer dalam memori yang bersaiz betul untuk memori flash, dan apabila buffer ini penuh, ia ditulis ke memori flash, menghapuskan sebarang data yang ada di lokasi tersebut. Terdapat juga `flushSfudBuffer` untuk menulis buffer yang tidak lengkap, kerana data yang ditangkap tidak akan menjadi gandaan tepat dari saiz granulariti, jadi bahagian akhir data perlu ditulis.
...
### Tugas - sediakan penangkapan audio
...
1. Tambahkan yang berikut di bahagian atas fail ini:
    ...
    | BUFFER_SIZE           | 128044 | Saiz buffer audio untuk ditangkap. Audio akan ditangkap sebagai fail WAV, yang mempunyai 44 byte header, kemudian 128,000 byte data audio (setiap sampel adalah 2 byte) |
    | ADC_BUF_LEN           | 1600   | Saiz buffer untuk digunakan untuk menangkap audio dari DMAC |
...
1. Tambahkan yang berikut untuk mendefinisikan kelas `Mic` yang boleh menangkap dari mikrofon:
    ...
    Kelas ini pada masa ini hanya mempunyai beberapa medan untuk menjejaki sama ada rakaman telah dimulakan, dan sama ada rakaman sedia untuk digunakan. Apabila DMAC disediakan, ia terus menulis ke buffer memori, jadi bendera `_isRecording` menentukan sama ada buffer ini harus diproses atau diabaikan. Bendera `_isRecordingReady` akan ditetapkan apabila 4 saat audio yang diperlukan telah ditangkap. Medan `_writer` digunakan untuk menyimpan data audio ke memori flash.
...
1. Tambahkan kod berikut ke bahagian `private` kelas `Mic`:
    ...
    Kod ini mendefinisikan kaedah `configureDmaAdc` yang mengkonfigurasi DMAC, menyambungkannya ke ADC dan menetapkannya untuk mengisi dua buffer bergantian, `_adc_buf_0` dan `_adc_buf_1`.
...
1. Di bawah ini, tambahkan kod berikut:
    ...
    Kod ini mendefinisikan header WAV sebagai struktur yang mengambil 44 byte memori. Ia menulis butiran kepada header ini tentang kadar fail audio, saiz, dan bilangan saluran. Header ini kemudian ditulis ke memori flash.

1. Di bawah kod ini, tambahkan yang berikut untuk mengisytiharkan kaedah yang akan dipanggil apabila buffer audio sedia untuk diproses:
    ...
    Buffer audio adalah array integer 16-bit yang mengandungi audio dari ADC. ADC mengembalikan nilai 12-bit tidak bertanda (0-1023), jadi nilai ini perlu ditukar kepada nilai 16-bit bertanda, dan kemudian ditukar kepada 2 byte untuk disimpan sebagai data binari mentah.

    Byte ini ditulis ke buffer memori flash. Penulisan bermula pada indeks 44 - ini adalah offset dari 44 byte yang ditulis sebagai header fail WAV. Apabila semua byte yang diperlukan untuk panjang audio yang diperlukan telah ditangkap, data yang tinggal ditulis ke memori flash.
...
1. Dalam bahagian `public` kelas `Mic`, tambahkan kod berikut:
    ...
    Kod ini akan dipanggil oleh DMAC untuk memberitahu kod anda untuk memproses buffer. Ia memeriksa bahawa terdapat data untuk diproses, dan memanggil kaedah `audioCallback` dengan buffer yang relevan.
...
1. Di luar kelas, selepas deklarasi `Mic mic;`, tambahkan kod berikut:
    ...
    `DMAC_1_Handler` akan dipanggil oleh DMAC apabila buffer sedia untuk diproses. Fungsi ini dikenalpasti melalui nama, jadi hanya perlu wujud untuk dipanggil.
...
1. Di atas fungsi `loop`, isytiharkan fungsi untuk memproses audio yang ditangkap. Buat masa ini fungsi ini tidak melakukan apa-apa, tetapi kemudian dalam pelajaran ini ia akan menghantar ucapan untuk ditukar kepada teks:
