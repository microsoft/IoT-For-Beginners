...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:06:29+00:00",
...
Dalam pelajaran sebelumnya, anda telah belajar cara menggunakan sensor GPS untuk menangkap data lokasi. Untuk memvisualisasikan lokasi trak yang membawa makanan dan perjalanannya, data ini perlu dihantar ke perkhidmatan IoT di awan dan disimpan di suatu tempat.
...
Sistem komputer berurusan dengan data, dan data ini datang dalam pelbagai bentuk dan saiz. Ia boleh berbeza daripada nombor tunggal, kepada teks yang banyak, kepada video dan imej, serta data IoT. Data biasanya boleh dibahagikan kepada dua kategori - *data berstruktur* dan *data tidak berstruktur*.
...
✅ Lakukan penyelidikan: Bolehkah anda memikirkan beberapa contoh lain data berstruktur dan tidak berstruktur?
...
* Untuk trak penghantaran yang mengangkut makanan ke gudang, anda mahukan data GPS serta data kelajuan dan pecutan untuk memastikan pemandu memandu dengan selamat, serta identiti pemandu dan data mula/berhenti untuk memastikan pematuhan pemandu dengan undang-undang tempatan mengenai waktu bekerja.
...
Data ini boleh berubah secara berterusan. Sebagai contoh, jika peranti IoT berada di kabin trak, maka data yang dihantar mungkin berubah apabila treler berubah, contohnya hanya menghantar data suhu apabila treler berpendingin digunakan.
...
Sebagai contoh, jika anda menyimpan butiran peribadi pengguna dalam jadual, anda akan mempunyai beberapa jenis ID unik dalaman bagi setiap pengguna yang digunakan dalam baris dalam jadual yang mengandungi nama dan alamat pengguna. Jika anda kemudian ingin menyimpan butiran lain tentang pengguna itu, seperti pembelian mereka, dalam jadual lain, anda akan mempunyai satu lajur dalam jadual baharu untuk ID pengguna itu. Apabila anda mencari pengguna, anda boleh menggunakan ID mereka untuk mendapatkan butiran peribadi mereka daripada satu jadual, dan pembelian mereka daripada yang lain.

Pangkalan data SQL sesuai untuk menyimpan data berstruktur, dan untuk memastikan data sepadan dengan skema anda.

✅ Jika anda belum pernah menggunakan SQL sebelum ini, luangkan masa untuk membaca mengenainya di [halaman SQL di Wikipedia](https://wikipedia.org/wiki/SQL).

Beberapa pangkalan data SQL yang terkenal ialah Microsoft SQL Server, MySQL, dan PostgreSQL.

✅ Lakukan penyelidikan: Baca tentang beberapa pangkalan data SQL ini dan keupayaannya.

#### Pangkalan Data NoSQL

Pangkalan data NoSQL dipanggil NoSQL kerana ia tidak mempunyai struktur tegar yang sama seperti pangkalan data SQL. Ia juga dikenali sebagai pangkalan data dokumen kerana ia boleh menyimpan data tidak berstruktur seperti dokumen.
...
### Tugasan - Hantar Data GPS ke IoT Hub

1. Buat IoT Hub baharu menggunakan tier percuma.
    ...
    Ingat untuk membuat Kumpulan Sumber baharu. Namakan Kumpulan Sumber baharu `gps-sensor`, dan IoT Hub baharu dengan nama unik berdasarkan `gps-sensor`, seperti `gps-sensor-<nama anda>`.

    > 💁 Jika anda masih mempunyai IoT Hub daripada projek sebelumnya, anda boleh menggunakannya semula. Ingat untuk menggunakan nama IoT Hub ini dan Kumpulan Sumber yang ia berada semasa membuat perkhidmatan lain.

1. Tambahkan peranti baharu ke IoT Hub. Namakan peranti ini `gps-sensor`. Ambil rentetan sambungan untuk peranti tersebut.

1. Kemas kini kod peranti anda untuk menghantar data GPS ke IoT Hub baharu menggunakan rentetan sambungan peranti daripada langkah sebelumnya.
...
Jika anda menggunakan Wio Terminal, ingat untuk menambah semua perpustakaan yang diperlukan, dan tetapkan masa menggunakan pelayan NTP. Kod anda juga perlu memastikan bahawa ia telah membaca semua data daripada port bersiri sebelum menghantar lokasi GPS, menggunakan kod sedia ada daripada pelajaran sebelumnya. Gunakan kod berikut untuk membina dokumen JSON:
...
Jalankan kod peranti anda dan pastikan mesej mengalir ke IoT Hub menggunakan arahan CLI `az iot hub monitor-events`.

## Laluan Panas, Suam, dan Sejuk

Data yang mengalir daripada peranti IoT ke awan tidak selalu diproses secara masa nyata. Sesetengah data memerlukan pemprosesan masa nyata, data lain boleh diproses sedikit masa kemudian, dan data lain boleh diproses jauh lebih lewat. Aliran data ke perkhidmatan yang berbeza yang memproses data pada masa yang berbeza dirujuk sebagai laluan panas, suam, dan sejuk.
...
Untuk menggunakan data laluan panas, kod anda akan bertindak balas kepada acara sebaik sahaja ia diterima oleh perkhidmatan awan anda.

### Laluan Suam

Laluan suam merujuk kepada data yang boleh diproses sejurus selepas diterima, contohnya untuk pelaporan atau analitik jangka pendek. Anda akan menggunakan data laluan suam untuk laporan harian tentang jarak perjalanan kenderaan, menggunakan data yang dikumpulkan pada hari sebelumnya.
...
Laluan sejuk merujuk kepada data sejarah, menyimpan data untuk jangka masa panjang untuk diproses bila-bila masa diperlukan. Sebagai contoh, anda boleh menggunakan laluan sejuk untuk mendapatkan laporan jarak perjalanan tahunan untuk kenderaan, atau menjalankan analitik pada laluan untuk mencari laluan yang paling optimum untuk mengurangkan kos bahan api.
...
Sebaik sahaja data mengalir ke IoT Hub anda, anda boleh menulis beberapa kod tanpa pelayan untuk mendengar acara yang diterbitkan ke titik akhir yang serasi dengan Event-Hub. Ini adalah laluan suam - data ini akan disimpan dan digunakan dalam pelajaran seterusnya untuk pelaporan perjalanan.
...
### Tugasan - Kendalikan Acara GPS Menggunakan Kod Tanpa Pelayan
...
1. Jalankan aplikasi fungsi anda untuk memastikan ia menerima acara daripada peranti GPS anda. Pastikan peranti IoT anda juga sedang berjalan dan menghantar data GPS.
...
![Logo Azure Storage](../../../../../translated_images/azure-storage-logo.605c0f602c640d482a80f1b35a2629a32d595711b7ab1d7ceea843250615ff32.ms.png)
...
Perkataan *Blob* bermaksud objek besar binari, tetapi telah menjadi istilah untuk sebarang data tidak berstruktur. Anda boleh menyimpan sebarang data dalam storan blob, daripada dokumen JSON yang mengandungi data IoT, kepada fail imej dan filem. Storan blob mempunyai konsep *kontena*, iaitu baldi bernama yang anda boleh simpan data di dalamnya, serupa dengan jadual dalam pangkalan data relasi. Kontena ini boleh mempunyai satu atau lebih folder untuk menyimpan blob, dan setiap folder boleh mengandungi folder lain, serupa dengan cara fail disimpan pada cakera keras komputer anda.

Anda akan menggunakan storan blob dalam pelajaran ini untuk menyimpan data IoT.

✅ Lakukan penyelidikan: Baca tentang [Azure Blob Storage](https://docs.microsoft.com/azure/storage/blobs/storage-blobs-overview?WT.mc_id=academic-17441-jabenn)

### Storan Jadual

Storan jadual membolehkan anda menyimpan data separa berstruktur. Storan jadual sebenarnya adalah pangkalan data NoSQL, jadi tidak memerlukan set jadual yang ditakrifkan terlebih dahulu, tetapi ia direka untuk menyimpan data dalam satu atau lebih jadual, dengan kunci unik untuk menentukan setiap baris.

✅ Lakukan penyelidikan: Baca tentang [Azure Table Storage](https://docs.microsoft.com/azure/storage/tables/table-storage-overview?WT.mc_id=academic-17441-jabenn)

### Storan Barisan

Storan barisan membolehkan anda menyimpan mesej sehingga 64KB dalam saiz dalam barisan. Anda boleh menambah mesej ke belakang barisan, dan membacanya dari depan. Barisan menyimpan mesej tanpa had selagi masih ada ruang storan, jadi ia membolehkan mesej disimpan untuk jangka masa panjang, kemudian dibaca apabila diperlukan. Sebagai contoh, jika anda ingin menjalankan tugas bulanan untuk memproses data GPS, anda boleh menambahkannya ke barisan setiap hari selama sebulan, kemudian pada akhir bulan memproses semua mesej dari barisan.

✅ Lakukan penyelidikan: Baca tentang [Azure Queue Storage](https://docs.microsoft.com/azure/storage/queues/storage-queues-introduction?WT.mc_id=academic-17441-jabenn)
...
✅ Lakukan penyelidikan: Baca tentang [Azure File Storage](https://docs.microsoft.com/azure/storage/files/storage-files-introduction?WT.mc_id=academic-17441-jabenn)
...
* Gunakan pengikatan fungsi output untuk mengikat nilai pulangan fungsi kepada storan blob dan membolehkan blob disimpan secara automatik.
...
### Tugasan - sambungkan kod tanpa pelayan anda ke storan
...
1. Tambahkan perkara berikut ke fail `requirements.txt` untuk memasang pakej Pip storan Azure:
    ...
    > Jika anda mendapat ralat, tingkatkan versi Pip anda dalam persekitaran maya kepada versi terkini dengan arahan berikut, kemudian cuba lagi:
    >
    > ```sh
...
1. Sebelum kaedah `main`, tambahkan fungsi pembantu berikut:
    ...
    Apabila kontena baharu dicipta, akses awam diberikan untuk menanyakan blob dalam kontena. Ini akan digunakan dalam pelajaran seterusnya untuk memvisualisasikan data GPS pada peta.
...
1. Tambahkan perkara berikut di bawah kod ini:
    ...
    Kod ini mencatat bahawa blob akan ditulis dengan butirannya, kemudian memuat naik badan blob sebagai kandungan blob baharu.
...
### Tugasan - sahkan blob yang dimuat naik
...
### Tugasan - lancarkan aplikasi Fungsi anda ke awan
...
Data GPS tidak sentiasa tepat, dan lokasi yang dikesan boleh tersasar beberapa meter, terutamanya di dalam terowong dan kawasan dengan bangunan tinggi.
