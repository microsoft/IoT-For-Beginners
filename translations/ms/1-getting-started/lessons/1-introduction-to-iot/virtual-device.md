...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:34:28+00:00",
...
Daripada membeli peranti IoT bersama sensor dan aktuator, anda boleh menggunakan komputer anda untuk mensimulasikan perkakasan IoT. Projek [CounterFit](https://github.com/CounterFit-IoT/CounterFit) membolehkan anda menjalankan aplikasi secara tempatan yang mensimulasikan perkakasan IoT seperti sensor dan aktuator, serta mengakses sensor dan aktuator tersebut melalui kod Python tempatan yang ditulis dengan cara yang sama seperti kod yang anda tulis pada Raspberry Pi menggunakan perkakasan fizikal.
...
1. Pasang Visual Studio Code (VS Code). Ini adalah editor yang akan anda gunakan untuk menulis kod peranti maya dalam Python. Rujuk [dokumentasi VS Code](https://code.visualstudio.com?WT.mc_id=academic-17441-jabenn) untuk arahan memasang VS Code.
...
Aplikasi Hello World untuk perkakasan IoT maya akan memastikan bahawa Python dan Visual Studio Code telah dipasang dengan betul. Ia juga akan menyambung ke CounterFit untuk sensor dan aktuator IoT maya. Ia tidak akan menggunakan sebarang perkakasan, hanya menyambung untuk membuktikan semuanya berfungsi.
...
Salah satu ciri kuat Python adalah keupayaan untuk memasang [pakej Pip](https://pypi.org) - ini adalah pakej kod yang ditulis oleh orang lain dan diterbitkan ke Internet. Anda boleh memasang pakej Pip ke komputer anda dengan satu arahan, kemudian menggunakan pakej tersebut dalam kod anda. Anda akan menggunakan Pip untuk memasang pakej untuk berkomunikasi dengan CounterFit.

Secara lalai, apabila anda memasang pakej, ia tersedia di mana-mana pada komputer anda, dan ini boleh menyebabkan masalah dengan versi pakej - seperti satu aplikasi bergantung pada satu versi pakej yang rosak apabila anda memasang versi baru untuk aplikasi lain. Untuk mengatasi masalah ini, anda boleh menggunakan [persekitaran maya Python](https://docs.python.org/3/library/venv.html), iaitu salinan Python dalam folder khusus, dan apabila anda memasang pakej Pip, ia hanya dipasang ke folder tersebut.

> 💁 Jika anda menggunakan Raspberry Pi, anda tidak menyediakan persekitaran maya pada peranti tersebut untuk menguruskan pakej Pip, sebaliknya anda menggunakan pakej global, kerana pakej Grove dipasang secara global oleh skrip pemasang.

#### Tugas - konfigurasi persekitaran maya Python
...
1. Setelah persekitaran maya diaktifkan, arahan `python` lalai akan menjalankan versi Python yang digunakan untuk mencipta persekitaran maya. Jalankan arahan berikut untuk mendapatkan versinya:
    ...
    Outputnya sepatutnya mengandungi perkara berikut:
    ...
    > 💁 Versi Python anda mungkin berbeza - selagi ia versi 3.6 atau lebih tinggi, anda sudah bersedia. Jika tidak, padamkan folder ini, pasang versi Python yang lebih baru dan cuba lagi.
...
1. Jika Terminal VS Code sudah berjalan semasa VS Code dimulakan, ia tidak akan mempunyai persekitaran maya yang diaktifkan di dalamnya. Cara paling mudah adalah mematikan terminal menggunakan butang **Kill the active terminal instance**:

    ![Butang Kill the active terminal instance di VS Code](../../../../../translated_images/vscode-kill-terminal.1cc4de7c6f25ee08f423f0ead714e61d069fac1eb2089e97b8a7bbcb3d45fe5e.ms.png)
...
1. Tambahkan kod berikut ke bahagian atas `app.py`:
    ...
    > 💁 Jika anda mempunyai aplikasi lain yang berjalan pada port 5000, anda boleh mengubah ini dengan mengemas kini port dalam kod, dan menjalankan CounterFit menggunakan `CounterFit --port <port_number>`, menggantikan `<port_number>` dengan port yang anda ingin gunakan.
...
1. Anda perlu melancarkan terminal VS Code baru dengan memilih butang **Create a new integrated terminal**. Ini kerana aplikasi CounterFit sedang berjalan dalam terminal semasa.
    ...
    ![Butang Create a new integrated terminal di VS Code](../../../../../translated_images/vscode-new-terminal.77db8fc0f9cd31824b0e49a201beafe4ae4616d6c7339992cb2819e789b3eff9.ms.png)

1. Dalam terminal baru ini, jalankan fail `app.py` seperti sebelumnya. Status CounterFit akan berubah menjadi **Connected** dan LED akan menyala.

    ![Counter Fit menunjukkan status sebagai connected](../../../../../translated_images/counterfit-connected.ed30b46d8f79b0921f3fc70be10366e596a89dca3f80c2224a9d9fc98fccf884.ms.png)
