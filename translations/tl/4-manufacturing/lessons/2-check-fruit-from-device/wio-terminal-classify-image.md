...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:07:02+00:00",
...
1. Sa ibaba ng mga include directives, ideklara ang isang instance ng `WifiClientSecure`:
    ...
    Ang class na ito ay naglalaman ng code upang makipag-ugnayan sa mga web endpoints gamit ang HTTPS.
...
1. Idagdag ang sumusunod na function sa `main.cpp`, sa itaas ng `buttonPressed` function.
    ...
    > 💁 Ang POST request ay para sa pagpapadala ng data, at pagkuha ng response. Mayroon ding ibang uri ng request tulad ng GET requests na kumukuha ng data. Ang GET requests ay ginagamit ng iyong web browser upang mag-load ng mga web page.
    ...
    Kung ang 200 ay naibalik, binabasa ang resulta mula sa HTTP client. Ito ay isang text response mula sa REST API na may resulta ng prediction bilang isang JSON document. Ang JSON ay nasa sumusunod na format:
    ...
    > 💁 Ang mga image classifier ay magbabalik ng percentages para sa lahat ng tags na ginamit. Ang bawat tag ay magkakaroon ng probability na ang imahe ay tumutugma sa tag na iyon.
...
1. Sa `buttonPressed` function, palitan ang code na nagse-save sa SD card ng tawag sa `classifyImage`, o idagdag ito pagkatapos ng imahe ay naisulat, ngunit **bago** ang buffer ay na-delete:
    ...
    > 💁 Kung papalitan mo ang code na nagse-save sa SD card, maaari mong linisin ang iyong code sa pamamagitan ng pagtanggal sa `setupSDCard` at `saveToSDCard` functions.

1. I-upload at patakbuhin ang iyong code. Itutok ang camera sa prutas at pindutin ang C button. Makikita mo ang output sa serial monitor:
