...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:07:44+00:00",
...
Sa nakaraang aralin, natutunan mo kung paano gamitin ang GPS sensor upang makuha ang data ng lokasyon. Upang magamit ang data na ito para maipakita ang lokasyon ng isang trak na may kargang pagkain, at ang paglalakbay nito, kailangang ipadala ito sa isang IoT service sa cloud, at pagkatapos ay itabi sa isang lugar.

Sa araling ito, matututunan mo ang iba't ibang paraan ng pagtatabi ng IoT data, at kung paano magtago ng data mula sa iyong IoT service gamit ang serverless code.
...
* [Pamahalaan ang mga GPS event gamit ang serverless code](../../../../../3-transport/lessons/2-store-location-data)
...
Ang mga computer system ay humahawak ng data, at ang data na ito ay may iba't ibang anyo at laki. Maaaring magmula ito sa mga simpleng numero, malalaking teksto, mga video at larawan, hanggang sa IoT data. Ang data ay karaniwang nahahati sa dalawang kategorya - *structured* na data at *unstructured* na data.

* **Structured na data** ay data na may malinaw na istruktura na hindi nagbabago at karaniwang tumutugma sa mga talahanayan ng data na may relasyon. Halimbawa nito ay ang mga detalye ng isang tao tulad ng pangalan, petsa ng kapanganakan, at address.

* **Unstructured na data** ay data na walang malinaw na istruktura, kabilang ang data na madalas magbago ng istruktura. Halimbawa nito ay mga dokumento tulad ng mga nakasulat na dokumento o spreadsheet.

✅ Mag-research: Makakaisip ka ba ng iba pang halimbawa ng structured at unstructured na data?

> 💁 Mayroon ding semi-structured na data na may istruktura ngunit hindi akma sa mga fixed na talahanayan ng data.
...
* Para sa mga delivery truck na nagdadala ng pagkain sa mga warehouse, gusto mo ng GPS data pati na rin ang bilis at acceleration data upang matiyak na ligtas ang pagmamaneho ng driver, pati na rin ang drive identity at start/stop data upang matiyak ang pagsunod sa mga lokal na batas sa oras ng trabaho.
* Para sa mga refrigerated truck, gusto mo rin ng temperature data upang matiyak na hindi masyadong mainit o malamig ang pagkain at hindi masira habang nasa biyahe.

Ang data na ito ay maaaring magbago nang tuloy-tuloy. Halimbawa, kung ang IoT device ay nasa cab ng trak, maaaring magbago ang data na ipinapadala nito habang nagbabago ang trailer, halimbawa ay magpadala lamang ng temperature data kapag ginagamit ang isang refrigerated trailer.
...
Ang data na ito ay nag-iiba mula sa sasakyan patungo sa sasakyan, ngunit lahat ito ay ipinapadala sa parehong IoT service para sa pagproseso. Ang IoT service ay kailangang magproseso ng unstructured na data na ito, itinatabi ito sa paraang maaaring hanapin o suriin, ngunit gumagana sa iba't ibang istruktura ng data na ito.

### SQL vs NoSQL Storage

Ang mga database ay mga serbisyo na nagbibigay-daan sa iyo upang magtago at mag-query ng data. Ang mga database ay may dalawang uri - SQL at NoSQL.

#### SQL Databases

Ang mga unang database ay Relational Database Management Systems (RDBMS), o relational database. Kilala rin ang mga ito bilang SQL databases dahil sa Structured Query Language (SQL) na ginagamit upang makipag-ugnayan sa mga ito upang magdagdag, mag-alis, mag-update, o mag-query ng data. Ang mga database na ito ay binubuo ng isang schema - isang malinaw na hanay ng mga talahanayan ng data, katulad ng spreadsheet. Ang bawat talahanayan ay may maraming pinangalanang mga column. Kapag nagpasok ka ng data, nagdaragdag ka ng isang row sa talahanayan, inilalagay ang mga halaga sa bawat column. Pinapanatili nito ang data sa isang napaka-mahigpit na istruktura - bagaman maaari mong iwanang walang laman ang mga column, kung nais mong magdagdag ng bagong column, kailangan mong gawin ito sa database, pinupunan ang mga halaga para sa mga umiiral na row. Ang mga database na ito ay relational - kung saan ang isang talahanayan ay maaaring magkaroon ng relasyon sa isa pa.
...
Halimbawa, kung itatabi mo ang mga personal na detalye ng isang user sa isang talahanayan, magkakaroon ka ng isang uri ng internal unique ID bawat user na ginagamit sa isang row sa isang talahanayan na naglalaman ng pangalan at address ng user. Kung nais mong itabi ang iba pang detalye tungkol sa user na iyon, tulad ng kanilang mga binili, sa isa pang talahanayan, magkakaroon ka ng isang column sa bagong talahanayan para sa ID ng user na iyon. Kapag hinanap mo ang isang user, maaari mong gamitin ang kanilang ID upang makuha ang kanilang personal na detalye mula sa isang talahanayan, at ang kanilang mga binili mula sa isa pa.

Ang SQL databases ay perpekto para sa pagtatabi ng structured na data, at para sa kapag nais mong tiyakin na ang data ay tumutugma sa iyong schema.

✅ Kung hindi mo pa nagagamit ang SQL, maglaan ng oras upang basahin ito sa [SQL page sa Wikipedia](https://wikipedia.org/wiki/SQL).
...
Ang NoSQL databases ay tinatawag na NoSQL dahil wala silang parehong mahigpit na istruktura ng SQL databases. Kilala rin ang mga ito bilang document databases dahil maaari silang magtago ng unstructured na data tulad ng mga dokumento.

> 💁 Sa kabila ng kanilang pangalan, ang ilang NoSQL databases ay nagbibigay-daan sa iyo upang gumamit ng SQL upang mag-query ng data.
...
Ang NoSQL databases ay walang pre-defined na schema na naglilimita kung paano itinatabi ang data, sa halip maaari kang magpasok ng anumang unstructured na data, karaniwang gamit ang JSON documents. Ang mga dokumentong ito ay maaaring ayusin sa mga folder, katulad ng mga file sa iyong computer. Ang bawat dokumento ay maaaring magkaroon ng iba't ibang field mula sa ibang mga dokumento - halimbawa kung itatabi mo ang IoT data mula sa iyong mga sasakyang pang-sakahan, ang ilan ay maaaring may field para sa accelerometer at speed data, ang iba ay maaaring may field para sa temperatura sa trailer. Kung magdadagdag ka ng bagong uri ng trak, tulad ng isa na may built-in na timbangan upang subaybayan ang timbang ng kargamento, ang iyong IoT device ay maaaring magdagdag ng bagong field na ito at maaari itong itabi nang walang anumang pagbabago sa database.
...
Sa araling ito, gagamit ka ng NoSQL storage upang itabi ang IoT data.

## Magpadala ng GPS Data sa isang IoT Hub

Sa nakaraang aralin, nakakuha ka ng GPS data mula sa isang GPS sensor na nakakonekta sa iyong IoT device. Upang itabi ang IoT data na ito sa cloud, kailangan mo itong ipadala sa isang IoT service. Muli, gagamitin mo ang Azure IoT Hub, ang parehong IoT cloud service na ginamit mo sa nakaraang proyekto.
...
Ang data na dumadaloy mula sa isang IoT device patungo sa cloud ay hindi laging napoproseso nang real time. Ang ilang data ay kailangang iproseso nang real time, ang iba ay maaaring iproseso nang kaunti pagkatapos matanggap, at ang iba pa ay maaaring iproseso nang mas matagal pa. Ang daloy ng data sa iba't ibang serbisyo na nagpoproseso ng data sa iba't ibang oras ay tinutukoy bilang hot, warm, at cold paths.
...
Ang warm path ay tumutukoy sa data na maaaring iproseso nang kaunti pagkatapos matanggap, halimbawa para sa pag-uulat o short-term analytics. Gagamitin mo ang warm path data para sa mga daily report sa mileage ng sasakyan, gamit ang data na nakalap noong nakaraang araw.

Ang warm path data ay itinatabi sa sandaling matanggap ito ng cloud service sa loob ng isang uri ng storage na mabilis ma-access.

### Cold Path

Ang cold path ay tumutukoy sa historical data, pagtatabi ng data para sa pangmatagalan upang iproseso kung kailan kinakailangan. Halimbawa, maaari mong gamitin ang cold path upang makuha ang mga taunang mileage report para sa mga sasakyan, o magsagawa ng analytics sa mga ruta upang mahanap ang pinaka-optimal na ruta para mabawasan ang gastos sa gasolina.

Ang cold path data ay itinatabi sa mga data warehouse - mga database na idinisenyo para sa pagtatabi ng malaking dami ng data na hindi magbabago at maaaring ma-query nang mabilis at madali. Karaniwan kang magkakaroon ng regular na trabaho sa iyong cloud application na tatakbo sa regular na oras bawat araw, linggo, o buwan upang ilipat ang data mula sa warm path storage patungo sa data warehouse.

✅ Isipin ang data na nakalap mo sa mga araling ito. Ito ba ay hot, warm, o cold path data?

## Pamahalaan ang GPS Events Gamit ang Serverless Code

Kapag ang data ay dumadaloy na sa iyong IoT Hub, maaari kang magsulat ng serverless code upang makinig sa mga event na inilathala sa Event-Hub compatible endpoint. Ito ang warm path - ang data na ito ay itatabi at gagamitin sa susunod na aralin para sa pag-uulat sa paglalakbay.
...
### Gawain - Pamahalaan ang GPS Events Gamit ang Serverless Code
...
> ⚠️ Maaari mong tingnan ang [mga tagubilin para sa paglikha ng Azure Functions Project mula sa proyekto 2, aralin 5](../../../2-farm/lessons/5-migrate-application-to-the-cloud/README.md#create-a-serverless-application) kung kinakailangan.
...
1. Patakbuhin ang iyong functions app upang masiguro na ito ay tumatanggap ng mga event mula sa iyong GPS device. Siguraduhin na ang iyong IoT device ay tumatakbo rin at nagpapadala ng GPS data.
...
Ang Azure Storage Accounts ay isang pangkalahatang serbisyo sa imbakan na maaaring mag-imbak ng data sa iba't ibang paraan. Maaari kang mag-imbak ng data bilang blobs, sa queues, sa tables, o bilang mga file, at lahat ng ito ay sabay-sabay.
...
Ang Table storage ay nagbibigay-daan sa iyo na mag-imbak ng semi-structured data. Ang Table storage ay isang NoSQL database, kaya hindi nito kinakailangan ang isang pre-defined na set ng mga table, ngunit ito ay dinisenyo upang mag-imbak ng data sa isa o higit pang mga table, na may mga unique key upang tukuyin ang bawat row.
...
Ang Queue storage ay nagbibigay-daan sa iyo na mag-imbak ng mga mensahe na hanggang 64KB ang laki sa isang queue. Maaari kang magdagdag ng mga mensahe sa likod ng queue, at basahin ang mga ito mula sa harap. Ang mga queue ay nag-iimbak ng mga mensahe nang walang hangganan hangga't may natitirang espasyo sa imbakan, kaya't pinapayagan nitong maimbak ang mga mensahe nang pangmatagalan at mabasa kapag kinakailangan. Halimbawa, kung nais mong magpatakbo ng buwanang trabaho upang iproseso ang GPS data, maaari kang magdagdag nito sa isang queue araw-araw sa loob ng isang buwan, at pagkatapos ay iproseso ang lahat ng mga mensahe mula sa queue sa katapusan ng buwan.
...
Ang iyong function app ay kailangang kumonekta sa blob storage upang mag-imbak ng mga mensahe mula sa IoT Hub. Mayroong 2 paraan upang gawin ito:
...
![Pagpapadala ng GPS telemetry mula sa isang IoT device papunta sa IoT Hub, pagkatapos sa Azure Functions sa pamamagitan ng event hub trigger, pagkatapos ay sine-save ito sa blob storage](../../../../../translated_images/save-telemetry-to-storage-from-functions.ed3b1820980097f143d9f0570072da11304c2bc7906359dfa075b4d9b253c20f.tl.png)
...
1. Gumawa ng Azure Storage account. Pangalanan ito ng tulad ng `gps<iyong pangalan>`.
...
1. Bago ang `main` method, idagdag ang sumusunod na helper function:
   ...
   Ang Python blob SDK ay walang helper method upang lumikha ng container kung wala ito. Ang code na ito ay maglo-load ng connection string mula sa `local.settings.json` file (o mula sa Application Settings kapag na-deploy na sa cloud), pagkatapos ay lilikha ng `BlobServiceClient` class mula rito upang makipag-ugnayan sa blob storage account. Pagkatapos ay iikot nito ang lahat ng mga container para sa blob storage account, hinahanap ang isa na may ibinigay na pangalan - kung makakahanap ito, ibabalik nito ang `ContainerClient` class na maaaring makipag-ugnayan sa container upang lumikha ng blobs. Kung wala itong makita, lilikha ito ng bagong container at ibabalik ang client para sa bagong container.
...
1. Idagdag ang sumusunod na code pagkatapos nito:
   ...
   Ang code na ito ay bumubuo ng katawan ng blob na isusulat sa blob storage. Ito ay isang JSON document na naglalaman ng device ID, ang oras na ang telemetry ay ipinadala sa IoT Hub, at ang GPS coordinates mula sa telemetry.
...
1. Idagdag ang sumusunod sa ibaba ng code na ito:
   ...
   Ang code na ito ay nagla-log na ang isang blob ay isusulat kasama ang mga detalye nito, pagkatapos ay ina-upload ang blob body bilang nilalaman ng bagong blob.
...
1. Upang makita ang mga blob na nilikha, maaari mong gamitin ang [Azure Storage Explorer](https://azure.microsoft.com/features/storage-explorer/?WT.mc_id=academic-17441-jabenn), isang libreng tool na nagbibigay-daan sa iyo upang tingnan at pamahalaan ang iyong mga storage account, o mula sa CLI.
   ...
   1. Upang gamitin ang CLI, una kailangan mo ng account key. Patakbuhin ang sumusunod na command upang makuha ang key na ito:
...
> ⚠️ Maaari mong tingnan ang [mga tagubilin para sa pag-deploy ng iyong Functions app mula sa proyekto 2, aralin 5](../../../2-farm/lessons/5-migrate-application-to-the-cloud/README.md#task---deploy-your-functions-app-to-the-cloud) kung kinakailangan.
## 🚀 Hamon

Ang GPS data ay hindi palaging eksakto, at ang mga lokasyong natutukoy nito ay maaaring may pagkakaiba ng ilang metro, lalo na sa mga tunnels at lugar na may matataas na gusali.
...
* Basahin ang tungkol sa structured data sa [Data model page sa Wikipedia](https://wikipedia.org/wiki/Data_model)
* Basahin ang tungkol sa semi-structured data sa [Semi-structured data page sa Wikipedia](https://wikipedia.org/wiki/Semi-structured_data)
* Basahin ang tungkol sa unstructured data sa [Unstructured data page sa Wikipedia](https://wikipedia.org/wiki/Unstructured_data)
