...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:41:03+00:00",
...
1. I-install ang Visual Studio Code (VS Code). Ito ang editor na gagamitin mo para magsulat ng code para sa iyong device gamit ang C/C++. Tingnan ang [VS Code documentation](https://code.visualstudio.com?WT.mc_id=academic-17441-jabenn) para sa mga tagubilin kung paano i-install ang VS Code.
...
1. I-install ang VS Code PlatformIO extension. Ito ay isang extension para sa VS Code na sumusuporta sa pag-program ng mga microcontroller gamit ang C/C++. Tingnan ang [PlatformIO extension documentation](https://marketplace.visualstudio.com/items?WT.mc_id=academic-17441-jabenn&itemName=platformio.platformio-ide) para sa mga tagubilin kung paano i-install ang extension na ito sa VS Code. Ang extension na ito ay umaasa sa Microsoft C/C++ extension upang gumana sa C at C++ code, at ang C/C++ extension ay awtomatikong mai-install kapag in-install mo ang PlatformIO.
...
Karaniwan kapag nagsisimula sa isang bagong programming language o teknolohiya, gumagawa ng isang 'Hello World' application - isang maliit na application na naglalabas ng text tulad ng `"Hello World"` upang ipakita na ang lahat ng tools ay maayos na naka-configure.
...
1. I-configure ang proyekto sa *Project Wizard*:
    ...
    1. Pangalanan ang iyong proyekto na `nightlight`
    ...
    Ang PlatformIO ay magda-download ng mga component na kailangan upang i-compile ang code para sa Wio Terminal at gagawa ng iyong proyekto. Maaaring tumagal ito ng ilang minuto.

### Suriin ang PlatformIO project

Ang VS Code explorer ay magpapakita ng ilang mga file at folder na ginawa ng PlatformIO wizard.
...
* `main.cpp` - ang file na ito sa `src` folder ay naglalaman ng entry point para sa iyong application. Buksan ang file na ito, at maglalaman ito ng sumusunod na code:
    ...
    Kapag nag-start ang device, ang Arduino framework ay tatakbo sa `setup` function nang isang beses, pagkatapos ay tatakbo ang `loop` function nang paulit-ulit hanggang sa ma-off ang device.
...
1. Palitan ang code upang tumugma sa sumusunod:
    ...
    Ang `setup` function ay nag-i-initialize ng koneksyon sa serial port - sa kasong ito, ang USB port na ginagamit upang ikonekta ang Wio Terminal sa iyong computer. Ang parameter na `9600` ay ang [baud rate](https://wikipedia.org/wiki/Symbol_rate) (kilala rin bilang Symbol rate), o bilis ng pagpapadala ng data sa serial port sa bits per second. Ang setting na ito ay nangangahulugan na 9,600 bits (0s at 1s) ng data ang ipinapadala bawat segundo. Pagkatapos nito, naghihintay ito hanggang maging handa ang serial port.

    Ang `loop` function ay nagpapadala ng linya na `Hello World!` sa serial port, kaya ang mga karakter ng `Hello World!` kasama ang isang bagong linya na karakter. Pagkatapos nito, natutulog ito ng 5,000 milliseconds o 5 segundo. Kapag natapos ang `loop`, ito ay tatakbo muli, at muli, at paulit-ulit habang naka-on ang microcontroller.

1. Ilagay ang iyong Wio Terminal sa upload mode. Kailangan mong gawin ito tuwing mag-u-upload ka ng bagong code sa device:

    1. Hilahin pababa nang dalawang beses nang mabilis ang power switch - babalik ito sa on position bawat beses.
...
1. I-build at i-upload ang code sa Wio Terminal
    ...
    1. Ang code ay i-compile at i-upload sa Wio Terminal
        ...
        > 💁 Kung gumagamit ka ng macOS, isang notification tungkol sa *DISK NOT EJECTED PROPERLY* ang lalabas. Ito ay dahil ang Wio Terminal ay na-mount bilang isang drive bilang bahagi ng flashing process, at ito ay nadidisconnect kapag ang compiled code ay naisusulat sa device. Maaari mong balewalain ang notification na ito.

    ⚠️ Kung makakakuha ka ng error tungkol sa upload port na hindi available, tiyaking nakakonekta ang Wio Terminal sa iyong computer, naka-on gamit ang switch sa kaliwang bahagi ng screen, at naka-set sa upload mode. Ang berdeng ilaw sa ibaba ay dapat naka-on, at ang asul na ilaw ay dapat pumipintig. Kung makakakuha ka pa rin ng error, hilahin pababa nang dalawang beses nang mabilis ang on/off switch upang pilitin ang Wio Terminal sa upload mode at subukang muli ang upload.

Ang PlatformIO ay may Serial Monitor na maaaring mag-monitor ng data na ipinapadala sa USB cable mula sa Wio Terminal. Pinapayagan ka nitong i-monitor ang data na ipinapadala ng `Serial.println("Hello World");` command.
...
> 💁 Makikita mo ang code na ito sa [code/wio-terminal](../../../../../1-getting-started/lessons/1-introduction-to-iot/code/wio-terminal) folder.
