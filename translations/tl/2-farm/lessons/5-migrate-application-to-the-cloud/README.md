...
✅ Balikan ang code na isinulat mo bilang server code na nakikinig sa mga mensahe sa MQTT. Paano kaya ito tatakbo sa cloud gamit ang serverless? Sa tingin mo, paano maaaring baguhin ang code upang suportahan ang serverless computing?

> 💁 Ang serverless model ay lumalawak na rin sa ibang cloud services bukod sa pagpapatakbo ng code. Halimbawa, may mga serverless databases na available sa cloud gamit ang serverless pricing model kung saan nagbabayad ka per request na ginawa laban sa database, tulad ng query o insert, karaniwang batay sa kung gaano karaming trabaho ang ginawa upang maibigay ang request. Halimbawa, ang isang simpleng select ng isang row laban sa primary key ay mas mura kaysa sa isang komplikadong operasyon na nagjo-join ng maraming table at nagbabalik ng libu-libong row.
...
> 💁 Sinusuportahan din ng Azure Functions ang custom handlers kaya maaari kang magsulat ng iyong functions sa anumang wika na sumusuporta sa HTTP requests, kabilang ang mga mas lumang wika tulad ng COBOL.
...
> 🎓 Sa Azure, ang storage na ginagamit ng Azure Functions ay isang Azure Storage Account. Ang mga account na ito ay maaaring mag-imbak ng mga file, blobs, data sa mga table, o data sa mga queue. Maaari mong ibahagi ang isang storage account sa maraming app, tulad ng isang Functions app at isang web app.
...
1. I-install ang mga kinakailangang Pip package gamit ang requirements file:
    ...
    > 💁 Ang mga kinakailangang Pip package ay kailangang nasa file na ito upang kapag dineploy ang Functions app sa cloud, masiguro ng runtime na mai-install ang tamang mga package.
...
1. Upang masubukan kung gumagana nang maayos ang lahat, maaari mong simulan ang Functions runtime. Patakbuhin ang sumusunod na command upang gawin ito:
    ...
    ```
> ⚠️ Kung makakita ka ng firewall notification, payagan ang access dahil kailangang mabasa at maisulat ng application na `func` sa iyong network.
...
* `function.json` - naglalaman ito ng configuration para sa trigger. Ang pangunahing configuration ay nasa seksyong tinatawag na `bindings`. Ang binding ay ang tawag sa koneksyon sa pagitan ng Azure Functions at iba pang Azure services. Ang function na ito ay may input binding sa isang event hub - kumokonekta ito sa event hub at tumatanggap ng data.
    ...
    > 💁 Maaari ka ring magkaroon ng output bindings upang ang output ng isang function ay maipadala sa ibang serbisyo. Halimbawa, maaari kang magdagdag ng output binding sa isang database at ibalik ang IoT Hub event mula sa function, at ito ay awtomatikong maipasok sa database.
  ...
  * `"connection": ""` - tinutukoy nito ang pangalan ng setting para basahin ang connection string. Kapag tumatakbo nang lokal, babasahin nito ang setting mula sa `local.settings.json` file.
    ...
    > 💁 Ang connection string ay hindi maaaring itago sa `function.json` file, kailangan itong basahin mula sa settings. Ito ay upang maiwasan ang aksidenteng paglalantad ng iyong connection string.
...
1. I-update ang halaga ng `"connection"` sa `function.json` file upang ituro sa bagong halaga na idinagdag mo sa `local.settings.json` file:
    ...
    > 💁 Tandaan - kailangan itong ituro sa setting, hindi naglalaman ng aktwal na connection string.
...
1. Siguraduhing hindi tumatakbo ang IoT Hub event monitor. Kung tumatakbo ito kasabay ng functions app, hindi makakonekta ang functions app at hindi makakakonsumo ng mga event.
    ...
    > 💁 Maraming app ang maaaring kumonekta sa IoT Hub endpoints gamit ang iba't ibang *consumer groups*. Ang mga ito ay tatalakayin sa susunod na leksyon.
...
> 💁 Makikita mo ang code na ito sa folder na [code/functions](../../../../../2-farm/lessons/5-migrate-application-to-the-cloud/code/functions).
...
1. Patakbuhin ang sumusunod na command upang gumawa ng storage account:
    ...
    ✅ Basahin ang tungkol sa pricing sa [Azure Storage Account pricing page](https://azure.microsoft.com/pricing/details/storage/?WT.mc_id=academic-17441-jabenn)
...
> 💁 Makikita mo ang isang halaga na naka-set na para sa `AzureWebJobsStorage`. Sa iyong file na `local.settings.json`, ito ay naka-set sa isang halaga upang gamitin ang local storage emulator. Kapag ginawa mo ang Functions App, ipinapasa mo ang storage account bilang parameter, at ito ay awtomatikong na-set sa setting na ito.
