...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:36:35+00:00",
...
Jika Anda menggunakan Raspberry Pi sebagai perangkat keras IoT Anda, ada dua pilihan - Anda dapat menyelesaikan semua pelajaran ini dan memprogram langsung di Pi, atau Anda dapat terhubung secara jarak jauh ke Pi 'headless' dan memprogram dari komputer Anda.
...
1. Hubungkan Grove Base Hat ke Pi Anda. Soket pada hat ini pas di atas semua pin GPIO pada Pi, meluncur hingga ke bawah pin untuk duduk dengan kokoh di dasar. Hat ini menutupi Pi.
...
1. Ikuti petunjuk dalam [panduan pengaturan Raspberry Pi](https://projects.raspberrypi.org/en/projects/raspberry-pi-setting-up) untuk menyiapkan Pi Anda, menghubungkannya ke keyboard/mouse/monitor, menghubungkannya ke jaringan WiFi atau ethernet Anda, dan memperbarui perangkat lunak.
...
1. Jalankan perintah berikut untuk menginstal semua pustaka yang diperlukan untuk perangkat keras Grove:
    ...
    Salah satu fitur kuat Python adalah kemampuan untuk menginstal [Pip packages](https://pypi.org) - ini adalah paket kode yang ditulis oleh orang lain dan dipublikasikan di Internet. Anda dapat menginstal paket Pip ke komputer Anda dengan satu perintah, lalu menggunakan paket tersebut dalam kode Anda.
    ...
    > 💁 Secara default, ketika Anda menginstal paket, paket tersebut tersedia di seluruh komputer Anda, dan ini dapat menyebabkan masalah dengan versi paket - seperti satu aplikasi bergantung pada satu versi paket yang rusak ketika Anda menginstal versi baru untuk aplikasi lain. Untuk mengatasi masalah ini, Anda dapat menggunakan [Python virtual environment](https://docs.python.org/3/library/venv.html), yang pada dasarnya adalah salinan Python dalam folder khusus, dan ketika Anda menginstal paket Pip, mereka hanya diinstal ke folder tersebut. Anda tidak akan menggunakan virtual environment saat menggunakan Pi Anda. Skrip instalasi Grove menginstal pustaka Python Grove secara global, jadi untuk menggunakan virtual environment, Anda perlu menyiapkan virtual environment lalu menginstal ulang pustaka Grove secara manual di dalam lingkungan tersebut. Lebih mudah menggunakan paket global, terutama karena banyak pengembang Pi akan mem-flash ulang kartu SD bersih untuk setiap proyek.
...
1. Instal Pylance. Ini adalah ekstensi untuk VS Code yang menyediakan dukungan bahasa Python. Lihat [dokumentasi ekstensi Pylance](https://marketplace.visualstudio.com/items?WT.mc_id=academic-17441-jabenn&itemName=ms-python.vscode-pylance) untuk petunjuk tentang cara menginstal ekstensi ini di VS Code.
...
1. Buka **Advanced Options** dengan menekan `Ctrl+Shift+X`. Opsi ini memungkinkan beberapa pra-konfigurasi Raspberry Pi OS sebelum diimajinasikan ke kartu SD.
    ...
    1. Jika Anda berencana untuk terhubung ke Pi melalui WiFi, centang kotak **Configure WiFi**, dan masukkan SSID dan kata sandi WiFi Anda, serta pilih negara WiFi Anda. Anda tidak perlu melakukan ini jika akan menggunakan kabel ethernet. Pastikan jaringan yang Anda hubungkan sama dengan yang digunakan komputer Anda.

    1. Centang kotak **Set locale settings**, dan atur negara serta zona waktu Anda.

    1. Pilih tombol **SAVE**.

1. Pilih tombol **WRITE** untuk menulis OS ke kartu SD. Jika Anda menggunakan macOS, Anda akan diminta memasukkan kata sandi Anda karena alat yang mendasari yang menulis gambar disk memerlukan akses istimewa.
...
Langkah berikutnya adalah mengakses Pi secara jarak jauh. Anda dapat melakukannya menggunakan `ssh`, yang tersedia di macOS, Linux, dan versi Windows terbaru.
...
1. Buka Terminal atau Command Prompt, dan masukkan perintah berikut untuk terhubung ke Pi:
    ...
    Jika Anda menggunakan Windows versi lama yang tidak memiliki `ssh` terinstal, Anda dapat menggunakan OpenSSH. Anda dapat menemukan petunjuk instalasi di [dokumentasi instalasi OpenSSH](https://docs.microsoft.com//windows-server/administration/openssh/openssh_install_firstuse?WT.mc_id=academic-17441-jabenn).
...
1. Ini akan menghubungkan Anda ke Pi dan meminta kata sandi.
    ...
    > 💁 Jika Anda tidak dapat terhubung menggunakan `raspberrypi.local`, maka Anda dapat menggunakan alamat IP Pi Anda. Lihat [dokumentasi alamat IP Raspberry Pi](https://www.raspberrypi.org/documentation/remote-access/ip-address.md) untuk petunjuk tentang beberapa cara mendapatkan alamat IP.
...
Setelah Pi dikonfigurasi, Anda dapat terhubung ke Pi menggunakan Visual Studio Code (VS Code) dari komputer Anda - ini adalah editor teks pengembang gratis yang akan Anda gunakan untuk menulis kode perangkat Anda dalam Python.
...
1. Ikuti petunjuk dalam [dokumentasi pengembangan jarak jauh VS Code menggunakan SSH](https://code.visualstudio.com/docs/remote/ssh?WT.mc_id=academic-17441-jabenn) untuk menginstal komponen yang diperlukan.

1. Mengikuti petunjuk yang sama, sambungkan VS Code ke Pi.

1. Setelah terhubung, ikuti petunjuk [mengelola ekstensi](https://code.visualstudio.com/docs/remote/ssh#_managing-extensions?WT.mc_id=academic-17441-jabenn) untuk menginstal [ekstensi Pylance](https://marketplace.visualstudio.com/items?WT.mc_id=academic-17441-jabenn&itemName=ms-python.vscode-pylance) secara jarak jauh ke Pi.
...
Aplikasi ini akan berada di dalam folder bernama `nightlight`, dan akan digunakan kembali dengan kode yang berbeda di bagian lain dari tugas ini untuk membangun aplikasi nightlight.
...
1. Dari Terminal VS Code, jalankan perintah berikut untuk menjalankan aplikasi Python Anda:
    ...
    > 💁 Anda mungkin perlu secara eksplisit memanggil `python3` untuk menjalankan kode ini jika Anda memiliki Python 2 yang terinstal selain Python 3 (versi terbaru). Jika Anda memiliki Python 2 yang terinstal, maka memanggil `python` akan menggunakan Python 2 alih-alih Python 3. Secara default, versi terbaru Raspberry Pi OS hanya memiliki Python 3 yang terinstal.
