...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T23:35:48+00:00",
...
1. Tambahkan include untuk file header WiFi dan file header config di bagian atas file `main.cpp`:
...
1. Panggil fungsi ini dari fungsi `setup` setelah koneksi serial dibuat:
...
1. Tambahkan metode berikut ke bagian `private`:
    ...
    Kode ini membangun URL untuk API penerbit token menggunakan lokasi sumber daya ucapan. Kemudian membuat `HTTPClient` untuk melakukan permintaan web, mengaturnya untuk menggunakan WiFi client yang dikonfigurasi dengan sertifikat endpoint token. Kode ini menetapkan kunci API sebagai header untuk panggilan. Kemudian melakukan permintaan POST untuk mendapatkan sertifikat, mencoba ulang jika terjadi kesalahan. Akhirnya token akses dikembalikan.

1. Ke bagian `public`, tambahkan metode untuk mendapatkan token akses. Ini akan diperlukan dalam pelajaran berikutnya untuk mengonversi teks menjadi ucapan.
...
1. Ke bagian `public`, tambahkan metode `init` yang mengatur token client:
    ...
    Ini menetapkan sertifikat pada WiFi client, lalu mendapatkan token akses.
...
1. Inisialisasi kelas `SpeechToText` di akhir fungsi `setup`, setelah panggilan `mic.init` tetapi sebelum `Ready` ditulis ke serial monitor:
...
1. Pada bagian sebelumnya dari pelajaran ini, audio direkam ke memori flash. Audio ini perlu dikirim ke REST API Layanan Ucapan, sehingga perlu dibaca dari memori flash. Audio tidak dapat dimuat ke dalam buffer di memori karena ukurannya terlalu besar. Kelas `HTTPClient` yang membuat panggilan REST dapat melakukan streaming data menggunakan Arduino Stream - sebuah kelas yang dapat memuat data dalam potongan kecil, mengirimkan potongan satu per satu sebagai bagian dari permintaan. Setiap kali Anda memanggil `read` pada stream, itu akan mengembalikan blok data berikutnya. Arduino stream dapat dibuat untuk membaca dari memori flash. Buat file baru bernama `flash_stream.h` di folder `src`, dan tambahkan kode berikut ke dalamnya:
...
1. Di bagian `private`, tambahkan metode berikut:
    ...
    Kode ini membaca dari memori flash di alamat saat ini dan menyimpan data dalam buffer. Kemudian meningkatkan alamat, sehingga panggilan berikutnya membaca blok memori berikutnya. Buffer diukur berdasarkan ukuran chunk terbesar yang akan dikirim `HTTPClient` ke REST API dalam satu waktu.

    > 💁 Menghapus memori flash harus dilakukan menggunakan ukuran grain, sedangkan membaca tidak.
...
1. Di bagian `public` dari kelas ini, tambahkan konstruktor:
    ...
    Konstruktor ini mengatur semua field untuk mulai membaca dari awal blok memori flash, dan memuat chunk data pertama ke dalam buffer.
...
1. Implementasikan metode `peek`. Metode ini mengembalikan data pada posisi saat ini tanpa memindahkan stream. Memanggil `peek` beberapa kali akan selalu mengembalikan data yang sama selama tidak ada data yang dibaca dari stream.
...
1. Implementasikan fungsi `available`. Fungsi ini mengembalikan berapa banyak byte yang dapat dibaca dari stream, atau -1 jika stream selesai. Untuk kelas ini, maksimum yang tersedia tidak akan lebih dari ukuran chunk HTTPClient. Ketika stream ini digunakan dalam HTTP client, fungsi ini dipanggil untuk melihat berapa banyak data yang tersedia, lalu meminta data sebanyak itu untuk dikirim ke REST API. Kita tidak ingin setiap chunk lebih besar dari ukuran chunk HTTP client, jadi jika lebih banyak yang tersedia, ukuran chunk dikembalikan. Jika lebih sedikit, maka yang tersedia dikembalikan. Setelah semua data di-streaming, -1 dikembalikan.
...
### Tugas - mengonversi ucapan menjadi teks

1. Ucapan dapat diubah menjadi teks dengan mengirimkan audio ke Layanan Ucapan melalui REST API. REST API ini memiliki sertifikat yang berbeda dengan penerbit token, jadi tambahkan kode berikut ke file header `config.h` untuk mendefinisikan sertifikat ini:
...
1. Tambahkan kode berikut ke bagian `public` dari kelas `SpeechToText` untuk mendefinisikan metode untuk mengonversi ucapan menjadi teks:
...
1. Beberapa header perlu diatur pada koneksi:
    ...
    Ini menetapkan header untuk otorisasi menggunakan token akses, format audio menggunakan sample rate, dan menetapkan bahwa klien mengharapkan hasil dalam format JSON.
...
1. Di bawah ini, tambahkan kode berikut:
    ...
    Jika kode respons adalah 200, yang berarti sukses, maka hasilnya diambil, didekode dari JSON, dan properti `DisplayText` diatur ke variabel `text`. Properti ini adalah tempat teks hasil ucapan dikembalikan.

    Jika kode respons adalah 401, maka token akses telah kedaluwarsa (token ini hanya berlaku selama 10 menit). Token akses baru diminta, dan panggilan dilakukan lagi.

    Jika tidak, kesalahan dikirim ke serial monitor, dan `text` dibiarkan kosong.
...
1. Di `main.cpp`, panggil metode baru `convertSpeechToText` ini di fungsi `processAudio`, lalu log hasil ucapan ke serial monitor:
...
1. Bangun kode ini, unggah ke Wio Terminal Anda, dan uji melalui serial monitor. Setelah Anda melihat `Ready` di serial monitor, tekan tombol C (yang di sisi kiri, paling dekat dengan sakelar daya), dan berbicaralah. 4 detik audio akan direkam, lalu diubah menjadi teks.
