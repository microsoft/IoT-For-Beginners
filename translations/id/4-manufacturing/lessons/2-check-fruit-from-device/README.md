...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-27T22:57:55+00:00",
...
Sensor kamera, seperti namanya, adalah kamera yang dapat Anda hubungkan ke perangkat IoT Anda. Kamera ini dapat mengambil gambar diam atau merekam video streaming. Beberapa kamera akan mengembalikan data gambar mentah, sementara yang lain akan mengompresi data gambar menjadi file gambar seperti JPEG atau PNG. Biasanya, kamera yang bekerja dengan perangkat IoT jauh lebih kecil dan memiliki resolusi lebih rendah dibandingkan yang biasa Anda gunakan, tetapi Anda juga dapat menemukan kamera beresolusi tinggi yang setara dengan ponsel kelas atas. Anda bahkan dapat menemukan berbagai lensa yang dapat diganti, pengaturan multi-kamera, kamera termal infra-merah, atau kamera UV.
...
Sebagian besar sensor kamera menggunakan sensor gambar di mana setiap piksel adalah fotodioda. Sebuah lensa memfokuskan gambar ke sensor gambar, dan ribuan atau jutaan fotodioda mendeteksi cahaya yang jatuh pada masing-masing dan merekamnya sebagai data piksel.
...
1. Dalam dialog *Publish Model*, atur *Prediction resource* ke sumber daya `fruit-quality-detector-prediction` yang Anda buat pada pelajaran sebelumnya. Biarkan nama sebagai `Iteration2`, dan pilih tombol **Publish**.
...
Anda mungkin menemukan bahwa hasil yang Anda dapatkan saat menggunakan kamera yang terhubung ke perangkat IoT Anda tidak sesuai dengan yang Anda harapkan. Prediksi tidak selalu seakurat saat menggunakan gambar yang diunggah dari komputer Anda. Hal ini terjadi karena model dilatih pada data yang berbeda dari yang digunakan untuk prediksi.

Untuk mendapatkan hasil terbaik dari pengklasifikasi gambar, Anda ingin melatih model dengan gambar yang semirip mungkin dengan gambar yang digunakan untuk prediksi. Jika Anda menggunakan kamera ponsel untuk menangkap gambar untuk pelatihan, misalnya, kualitas gambar, ketajaman, dan warna akan berbeda dengan kamera yang terhubung ke perangkat IoT.

![2 gambar pisang, satu dengan resolusi rendah dan pencahayaan buruk dari perangkat IoT, dan satu lagi dengan resolusi tinggi dan pencahayaan baik dari ponsel](../../../../../translated_images/banana-picture-compare.174df164dc326a42cf7fb051a7497e6113c620e91552d92ca914220305d47d9a.id.png)
...
✅ Apa lagi yang mungkin menyebabkan gambar yang diambil oleh perangkat IoT Anda menghasilkan prediksi yang salah? Pikirkan tentang lingkungan tempat perangkat IoT mungkin digunakan, faktor apa saja yang dapat memengaruhi gambar yang diambil?

Untuk meningkatkan model, Anda dapat melatih ulang menggunakan gambar yang diambil dari perangkat IoT.

### Tugas - meningkatkan model

1. Klasifikasikan beberapa gambar buah matang dan tidak matang menggunakan perangkat IoT Anda.

1. Di portal Custom Vision, latih ulang model menggunakan gambar pada tab *Predictions*.

    > ⚠️ Anda dapat merujuk [instruksi untuk melatih ulang pengklasifikasi Anda di pelajaran 1 jika diperlukan](../1-train-fruit-detector/README.md#retrain-your-image-classifier).
...
1. Perbarui URL endpoint di kode Anda, dan jalankan kembali aplikasi.
...
Cobalah mengubah resolusi gambar dalam kode perangkat Anda dan lihat apakah itu membuat perbedaan pada kualitas gambar. Juga coba ubah pencahayaan.

Jika Anda ingin membuat perangkat produksi untuk dijual ke petani atau pabrik, bagaimana Anda memastikan perangkat tersebut memberikan hasil yang konsisten setiap saat?

## Kuis Setelah Pelajaran

[Kuis Setelah Pelajaran](https://black-meadow-040d15503.1.azurestaticapps.net/quiz/32)

## Tinjauan & Studi Mandiri

Anda melatih model Custom Vision Anda menggunakan portal. Ini bergantung pada ketersediaan gambar - dan dalam dunia nyata, Anda mungkin tidak dapat memperoleh data pelatihan yang sesuai dengan apa yang ditangkap oleh kamera pada perangkat Anda. Anda dapat mengatasi ini dengan melatih langsung dari perangkat Anda menggunakan API pelatihan, untuk melatih model menggunakan gambar yang diambil dari perangkat IoT Anda.

* Bacalah tentang API pelatihan di [panduan memulai cepat menggunakan Custom Vision SDK](https://docs.microsoft.com/azure/cognitive-services/custom-vision-service/quickstarts/image-classification?WT.mc_id=academic-17441-jabenn&tabs=visual-studio&pivots=programming-language-python)
