...
CO_OP_TRANSLATOR_METADATA:
{
  ...
  "translation_date": "2025-08-28T00:05:17+00:00",
...
[Kuis Sebelum Pelajaran](https://black-meadow-040d15503.1.azurestaticapps.net/quiz/23)

## Pendahuluan

Pada pelajaran sebelumnya, Anda telah mempelajari cara menggunakan sensor GPS untuk menangkap data lokasi. Untuk memvisualisasikan lokasi truk yang membawa makanan dan perjalanannya, data ini perlu dikirim ke layanan IoT di cloud, lalu disimpan di suatu tempat.
...
* **Data terstruktur** adalah data dengan struktur yang jelas dan kaku yang tidak berubah, biasanya berupa tabel data dengan hubungan. Contohnya adalah detail seseorang termasuk nama, tanggal lahir, dan alamat.
...
* Untuk truk pengiriman yang mengangkut makanan ke gudang, Anda ingin data GPS serta data kecepatan dan percepatan untuk memastikan pengemudi mengemudi dengan aman, serta data identitas pengemudi dan mulai/berhenti untuk memastikan kepatuhan terhadap hukum kerja lokal.
* Untuk truk berpendingin, Anda juga ingin data suhu untuk memastikan makanan tidak terlalu panas atau dingin sehingga tidak rusak selama perjalanan.

Data ini dapat berubah terus-menerus. Misalnya, jika perangkat IoT berada di kabin truk, maka data yang dikirimkan dapat berubah saat trailer berubah, misalnya hanya mengirimkan data suhu saat trailer berpendingin digunakan.
...
> 💁 Meskipun namanya, beberapa database NoSQL memungkinkan Anda menggunakan SQL untuk melakukan kueri data.
...
Pada pelajaran sebelumnya, Anda menangkap data GPS dari sensor GPS yang terhubung ke perangkat IoT Anda. Untuk menyimpan data IoT ini di cloud, Anda perlu mengirimkannya ke layanan IoT. Sekali lagi, Anda akan menggunakan Azure IoT Hub, layanan cloud IoT yang sama yang Anda gunakan dalam proyek sebelumnya.
...
1. Buat IoT Hub baru menggunakan tier gratis.
    ...
    > 💁 Jika Anda masih memiliki IoT Hub dari proyek sebelumnya, Anda dapat menggunakannya kembali. Ingatlah untuk menggunakan nama IoT Hub ini dan Resource Group tempat ia berada saat membuat layanan lainnya.
...
1. Tambahkan yang berikut ini ke file `requirements.txt` untuk menginstal paket Pip penyimpanan Azure:
...
1. Sebelum metode `main`, tambahkan fungsi pembantu berikut:
    ...
    SDK blob Python tidak memiliki metode pembantu untuk membuat container jika tidak ada. Kode ini akan memuat string koneksi dari file `local.settings.json` (atau Pengaturan Aplikasi setelah diterapkan ke cloud), lalu membuat kelas `BlobServiceClient` dari ini untuk berinteraksi dengan akun penyimpanan blob. Kemudian kode ini akan melingkari semua container untuk akun penyimpanan blob, mencari satu dengan nama yang disediakan - jika menemukannya, kode akan mengembalikan kelas `ContainerClient` yang dapat berinteraksi dengan container untuk membuat blob. Jika tidak menemukannya, maka container akan dibuat dan klien untuk container baru akan dikembalikan.

    Ketika container baru dibuat, akses publik diberikan untuk menanyakan blob dalam container. Ini akan digunakan dalam pelajaran berikutnya untuk memvisualisasikan data GPS di peta.
...
1. Tambahkan kode berikut setelah ini:
    ...
    Ini membangun isi blob yang akan ditulis ke penyimpanan blob. Ini adalah dokumen JSON yang berisi ID perangkat, waktu telemetri dikirim ke IoT Hub, dan koordinat GPS dari telemetri.

    > 💁 Penting untuk menggunakan waktu antrean pesan sebagai lawan dari waktu saat ini untuk mendapatkan waktu pesan dikirim. Pesan mungkin berada di hub untuk sementara waktu sebelum diambil jika Aplikasi Fungsi tidak berjalan.

1. Tambahkan yang berikut ini di bawah kode ini:
...
1. Jalankan aplikasi Fungsi. Anda akan melihat blob ditulis untuk semua acara GPS di output:
    ...
    > 💁 Pastikan Anda tidak menjalankan monitor acara IoT Hub pada saat yang sama.
...
1. Untuk melihat blob yang dibuat, Anda dapat menggunakan [Azure Storage Explorer](https://azure.microsoft.com/features/storage-explorer/?WT.mc_id=academic-17441-jabenn), alat gratis yang memungkinkan Anda melihat dan mengelola akun penyimpanan Anda, atau dari CLI.
    ...
    1. Unduh salah satu blob menggunakan perintah berikut:
        ...
        Ganti `<blob_name>` dengan nama lengkap dari kolom `Name` dari output langkah terakhir, termasuk nama folder. Ganti `<file_name>` dengan nama file lokal untuk menyimpan blob.
...
### Tugas - terapkan aplikasi Fungsi Anda ke cloud

Sekarang aplikasi Fungsi Anda berfungsi, Anda dapat menerapkannya ke cloud.
...
1. Buat aplikasi Azure Functions baru, menggunakan akun penyimpanan yang Anda buat sebelumnya. Beri nama seperti `gps-sensor-` dan tambahkan pengidentifikasi unik di akhir, seperti beberapa kata acak atau nama Anda.
    ...
    > ⚠️ Anda dapat merujuk ke [instruksi untuk membuat aplikasi Fungsi dari proyek 2, pelajaran 5](../../../2-farm/lessons/5-migrate-application-to-the-cloud/README.md#task---create-the-cloud-resources) jika diperlukan.
...
1. Terapkan aplikasi Fungsi lokal Anda ke cloud.
...
Pikirkan bagaimana navigasi satelit dapat mengatasi masalah ini? Data apa yang dimiliki sat-nav Anda yang memungkinkan prediksi lokasi yang lebih baik?
